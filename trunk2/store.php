 ...
 * ============================================================================
 * $Id: store.php 5566 2008-08-18 05:05:40Z Scottye $
 */
...
class StoreController extends ControllerFrontend
{
    ...
    function view()
    {
        ...
        if (!($cache_id && $this->is_cached($cache_id)))
        {
            ...
            $this->assign('store_intro', Editor::parse($this->conf('store_intro', $store_id)));
            ...
            elseif ($cate_id > 0)
            {
                ...
                if ($info['parent_id'])
                {
                    ...
                    $location_data[] = array('name'=>$info1['cate_name'], 'url'=>'index.php?' . $url . '&amp;cate_id=' . $info1['cate_id'] . '&amp;cate_name=' . urlencode($info1['cate_name']));
                }
    ...
    function credit()
    {
        ...
        $cache_id = 'store_credit_' . $store_id;
        if (!$this->is_cached($cache_id))
        {
            $store_data = $this->get_store_data($store_id);
            if ($store_data == -1)
    ...
    function get_tabs($act, $store_id)
    {
        ...
        $tabs = array();
        //首页
        $tabs[] = array('class' => $act == 'view' ? 'first selected' : 'first' , 'href' => 'index.php?app=store&amp;store_id=' . $store_id, 'text' => $this->conf('store_name', $store_id));
        $art_id = isset($_GET['art_id']) ?  intval($_GET['art_id']) : -1;
        //文章
        foreach ($art_list as $v)
        {
            $tabs[] = array('class' => $art_id == $v['article_id'] ? 'selected' : '' , 'href' => 'index.php?app=store&amp;store_id=' . $store_id . '&amp;act=article&amp;art_id=' . $v['article_id'], 'text' => $v['title']);
        }
        //信用
        $tabs[] = array('class' => $act == 'credit' ? 'selected' : '', 'href' => 'index.php?app=store&amp;store_id=' . $store_id . '&amp;act=credit', 'text' => $this->lang('credit'));
        //留言
        $tabs[] = array('class' => $act == 'comment' ? 'last selected' : 'last', 'href' => 'index.php?app=store&amp;act=comment&amp;store_id=' . $store_id, 'text' => $this->lang('store_comment'));
    ...
    function get_store_data($store_id)
    {
        ...
        if (empty($store) || $store['is_open'] != 1 || $this->conf('store_status', $store_id) == 0)
        {
    ...
    function _get_location_data($store_id, $tail = false)
    {
        ...
        if ($tail)
        {
            $location_data[] = array('name' => $this->conf('store_name', $store_id), 'url' => 'index.php?app=store&amp;store_id=' . $store_id);
            if ($tail !== true)
            {
