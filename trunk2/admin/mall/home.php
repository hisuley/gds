 ...
 * ============================================================================
 * $Id: home.php 5623 2008-08-21 02:27:10Z Weberliu $
 */
...
class HomeController extends HomeBaseController
{
    ...
    function update_site_information()
    {
        ...
        if(empty($update_time) || ($timestamp - $update_time > 3600 * 4))
        {
            ...
            $stat_info = array();
            $stat_info['last_month_page_view']    = get_page_view();
            $stat_info['last_month_order_amount'] = get_order_count('last_month', 'finish', 'amount');
            $stat_info['last_month_order_count']  = get_order_count('last_month');
            $stat_info['last_month_store_count']  = get_store_count('last_month');
        ...
        //发布使用
        return '<sc'.'ript language="Jav'.'aScript" src="ht'.'tp:/'.'/cus'.'tome'.'r.disc'.'uz.n'.'et/n'.'ews'.'.p'.'hp?'.'update='.rawurlencode(base64_encode($data)).'&md5hash='.substr(md5($_SERVER['HTTP_USER_AGENT'].implode('', $update).$timestamp), 8, 8).'&timestamp='.$timestamp.'"></s'.'cri'.'pt>';
