 ...
 * ============================================================================
 * $Id: ctl.template.php 5530 2008-08-14 06:46:16Z Liupeng $
 */
...
class TemplateController extends ControllerBackend
{
    ...
    */
    ...
    function edit()
    {
        ...
        if (empty($_SESSION['store_id']))
        {
            ...
            {
                case 'homepage':
                    $this->redirect('./index.php?app=mall&edit_mode=1');
                break;
                ...
                case 'goods_list':
                    ...
                    if (empty($cate_id))
                    {
                        $this->show_message('no_cate');
                        return;
                    }
                    $this->redirect('./index.php?app=category&edit_mode=1&mall_cate_id='.$cate_id);
                break;
            }
        }
        else
        {
            ...
            {
                case 'homepage':
                    $this->redirect('./index.php?app=store&edit_mode=1&store_id='.$this->store_id);
                break;

                case 'goods_list':
                    $cate_id = $GLOBALS['db']->getOne("SELECT cate_id FROM `ecm_category` WHERE store_id='$this->store_id' LIMIT 1");
                    $this->redirect('./index.php?app=store&edit_mode=1&cate_id='.$cate_id . '&store_id='.$this->store_id);
                    if (empty($cate_id))
                    {
                        $this->show_message('no_cate');
                        return;
                ...
                case 'goods':
                    ...
                    if (empty($goods_id))
                    {
                        $this->show_message('no_goods', 'close_window', 'javascript:window.close();');
                        return;
                    }

                    $this->redirect('./index.php?app=goods&edit_mode=1&id='.$goods_id);
                break;
            }
        }
    }
    ...
    */
    ...
    {
        $sql = "SELECT * FROM `ecm_custom_modules` WHERE store_id='$this->store_id'";
    ...
    **/
    function add_module()
    {
        ...
        if ($GLOBALS['db']->autoExecute('`ecm_custom_modules`', $info))
        {
