 ...
 * ============================================================================
 * $Id: inc.stat.php 5618 2008-08-20 08:48:31Z Liupeng $
 */
...
function get_goods_count($time_range = 'unlimited', $condition = '', $store_id = 0)
{
    ...
    {
    case 'oos':
        $sql = 'SELECT COUNT(*) as c FROM `ecm_goods` g LEFT JOIN `ecm_goods_spec` s ON g.goods_id=s.goods_id WHERE 1';
        $condition_limit = ' AND s.stock=0';
    break;
    default:
    ...
    $sql .= $time_limit . $condition_limit;
...
{
    !$store_id && $store_id = $_SESSION['store_id'];
    $sql = 'SELECT %s as c FROM `ecm_order_info` WHERE 1';
    if ($store_id)
    {
        $sql .= ' AND store_id = ' . intval($store_id);
    }
