...
function addFileItem(file) {
  document.getElementById('file-list-window').style.display = '';
  var fileList = document.getElementById('file-list');
  var div = new bkElement('div');
  div.id = 'editor_file_' + file.file_id;
  var _input = new bkElement('input');
  var _file_size  = new bkElement('span');
  var _file_name  = new bkElement('span');
  var _del_button = new bkElement('a');
  var _append_button = new bkElement('a');
  _file_size.style.marginLeft = '15px';
  _file_name.style.marginLeft = '15px';
  _del_button.innerHTML = lang.ubb_delete_attach;
  _del_button.href  = 'javascript:delFile('+file.file_id+');';
  _append_button.href = 'javascript:insertFile('+file.file_id+',\''+file.file_name+'\');'
  _append_button.innerHTML = lang.ubb_insert_attach;
  _input.type = 'hidden';
  _input.name = 'file_id[]';
  _input.value = file.file_id;
  _file_size.innerHTML = lang.ubb_file_size + ':' + file.file_size;
  var arr = file.file_name.split("/");
  var basename = arr[arr.length-1];
  _file_name.innerHTML = lang.ubb_file_name + ':' + basename;
  div.appendChild(_input);
  div.appendChild(_file_name);
  div.appendChild(_file_size);
  div.appendChild(document.createTextNode("   "));
  div.appendChild(_append_button);
  div.appendChild(document.createTextNode(" "));
  div.appendChild(_del_button);
  fileList.appendChild(div);
}

function delFile(file_id) {
  Ajax.call(EDITOR_FILE_HANDLER + '?ajax=1&aid=' + file_id, function (r) {
    if (r.done) {
      Element.remove($('editor_file_' + file_id));
    } else {
      alert(r.msg);
    }
  });
...
function insertFile(fileId, filePath) {
  var frames = document.getElementsByTagName("iframe");
  
  for(var i=0; i<frames.length; i++) {
    if (frames[i].className.indexOf("nicEdit-frame")>=0) {
      var img = new frames[i].contentWindow.Image();
      img.src = filePath;
      try
      {
        frames[i].contentWindow.document.body.appendChild(img);
      } catch(ex)
      {
...
*/
    ...
    'ol': {
      name: __(lang.order_list),
      command: 'insertorderedlist',
      tags: ['OL']
    },
    'ul': {
      name: __(lang.unorder_list),
...
,
    ...
    'outdent': {
      name: __(lang.outdent),
      command: 'outdent',
      noActive: true
    },
    'hr': {
      name: __(lang.horizontal_rule),
      command: 'insertHorizontalRule',
      noActive: true
    }
  },
  iconsPath: 'editor/niceditoricons.gif',
  buttonList: ['bold', 'italic', 'underline','strikethrough', 'left', 'center', 'right', 'ol', 'ul', 'fontSize', 'fontFamily', 'fontFormat', 'indent', 'outdent', 'link', 'unlink', 'image'],
  iconList: {
    "xhtml": 1,
    "bgcolor": 2,
    "forecolor": 3,
...
var nicEditor = bkClass.extend({
  ...
  panelInstance: function(B, C) {
    ...
    if (B.getStyle("width").indexOf("%"))
    {
        B.setStyle({width:"600px"});
    }
    var A = new bkElement("DIV").setStyle({
      width: parseInt(B.getStyle("width")) + "px"
    }).appendBefore(B);
    this.setPanel(A);
...
var nicEditorInstance = bkClass.extend({
  ...
  construct: function(F, D, C) {
    ...
    if (G || this.options.hasPanel) {
      ...
      var FW = new bkElement("DIV").setStyle({
        width: (newX - 8) + "px",
        margin: "4px",
        textAlign: "left",
        display: 'none'}).addClass("file").appendTo(this.editorContain);
      FW.id = 'file-list-window';
      var bTitle = document.createElement('h3');
      bTitle.style.cssText = 'color:#5A5A5A; font-size:12px; margin:0px; padding:3px; background:#eee;';
      bTitle.className = 'nicEdit-ftitle';
      bTitle.innerHTML = lang.ubb_mng_attachs;
      var fileList = document.createElement('div');
      fileList.id = 'file-list';
      FW.appendChild(bTitle);
      FW.appendChild(fileList);
      F.setStyle({
        display: "none"
      });
      ...
      if (G) {
        if (this.ne.options.bbCode) {
          A.setContent(F.value.replace(/(\r)?\n/g, "[br]"));
        } else {
...
var nicEditorIFrameInstance = nicEditorInstance.extend({
  ...
  init: function() {
    ...
    if (this.copyElm) {
      try {
        if (this.elm.offsetWidth > 0)
        {
          this.elmFrame.setStyle({
            width: (this.elm.offsetWidth - 1) + "px"
          });
        }
      } catch(ex) {}
    }
  ...
  initFrame: function() {
    ...
    this.initialContent = this.initialContent == "<br />" ? '' : this.initialContent;
    var loadedPlugins = this.ne.loadedPlugins;
    this.elmFrame.style.height = this.initialHeight + "px";
    
    if (!this.loaded) {
      for(var i=0; i < loadedPlugins.length; i++) {
        if (loadedPlugins[i].fromBBCode && this.ne.options.bbCode) {
          this.initialContent = loadedPlugins[i].fromBBCode(this.initialContent);
        }
      }
    }
    ...
    try {
      ...
      this.frameDoc.open();
      this.frameDoc.write('<html><head></head><body id="nicEditContent" style="padding:5px; margin: 0 !important; background-color: transparent !important;">' + this.initialContent + "</body></html>");
      this.frameDoc.close();
    } catch(ex) {
      if (!bkLib.isMSIE) {
        this.frameDoc.body.style.cssText = "padding:5px; margin: 0 !important; background-color: transparent !important;";
        //this.frameDoc.addEvent("click", this.initFrame.closureListener(this));
        Event.observe(this.frameDoc, 'click', this.initFrame.closureListener(this));
        this.frameDoc.body.innerHTML = this.initialContent;
        return;
      }
    }    
    ...
    this.heightUpdate();
    this.frameDoc = $BK(this.elmFrame.contentWindow.document);
    var obj = this.frameDoc.addEvent("mousedown", this.selected.closureListener(this));
    var obj1 = obj.addEvent("keyup", this.heightUpdate.closureListener(this));
    var obj2 = obj1.addEvent("keydown", this.keyDown.closureListener(this)).addEvent("keyup", this.selected.closure(this));
  ...
  getSel: function() {
    return (this.frameWin.getSelection) ? this.frameWin.getSelection() : this.frameDoc.selection
  },
  heightUpdate: function() {
    
    /*
this.elmFrame.style.height = "0px";
    var A = this.frameContent.scrollHeight;
    this.elmFrame.style.height = Math.max(A, this.initialHeight) + "px";
*/
  },
  nicCommand: function(B, A) {
    if (B=="indent" && !bkLib.isMSIE){
      var range = this.getRng();
      var s = document.createElement("blockquote");
...
var nicEditorFontSizeSelect = nicEditorSelect.extend({
  ...
  init: function() {
    this.setDisplay(lang.font_size);
    for (itm in this.sel) {
...
var nicEditorFontFamilySelect = nicEditorSelect.extend({
  ...
  init: function() {
    this.setDisplay(lang.font_family);
    for (itm in this.sel) {
...
var nicEditorColorButton = nicEditorAdvancedButton.extend({
  ...
  colorSelect: function(color) {
    this.ne.nicCommand("foreColor", color);
...
var nicImageButton = nicEditorAdvancedButton.extend({
  ...
  addPane: function() {
    ...
    this.addForm({
      ...
      upload_files: {
        type: "file",
        txt: lang.upload_file,
        style: {
          width: "180px"
        }
      }
    }, this.im);
    if (!this.ne.options.allowUpload) {
      $BK(this.inputs.upload_files.parentNode).remove();
    }
    if (!document.getElementById("__editorUploadFrame")) {
      var uploadFrameOuter = document.createElement("div");
      uploadFrameOuter.id = "uploadFrameOuter";
      uploadFrameOuter.style.display = "none";
      uploadFrameOuter.innerHTML = "<iframe id='__editorUploadFrame' name='__editorUploadFrame'></iframe>"
      document.body.appendChild(uploadFrameOuter);
      var iframe = document.getElementById("__editorUploadFrame");
     
      var d = new Date();
      this.funcName = "_callback" + (d.getTime());
      var self = this;
      window[this.funcName] = function(obj) {
        if (!obj.done) {
          alert(obj.msg);
        } else {
          self.inputs.src.value = obj.retval.file_path;
          resetFileInput(self.inputs.upload_files);
          obj.retval.file_name = obj.retval.file_path;
          addFileItem(obj.retval);
          self.submit();
        }
      };
  ...
  submit: function(B) {
    if (this.inputs.upload_files.value != "") {
      var uploadForm = document.getElementById("uploadForm");
      if (uploadForm) {
        uploadForm.parentNode.removeChild(uploadForm)
      }
      document.getElementById("uploadFrameOuter").innerHTML += "<form id='uploadForm' target='__editorUploadFrame' enctype='multipart/form-data' method='post' action='editor/attachment.php?callback="+ this.funcName +"'></form>";
      uploadForm = document.getElementById("uploadForm");
      uploadForm.appendChild(this.inputs.upload_files);
      uploadForm.submit();
      return;
    }
    var C = this.inputs.src.value;
    if (C == "" || C == "http://") {
      alert(lang.url_not_empty);
      return false;
    }
    this.removePane();
    if (!this.im) {
...
var nicXHTML = bkClass.extend({
  ...
  noShort: ["style", "title", "script", "textarea", "a", "iframe"],
  cssReplace: {
  ...
  toXHTML: function(elem, noParent) {
      ...
      switch(nodeType) {
        ...
        case 1:
          switch (nodeName) {
            ...
            case "ul":
              if (elem.firstChild.nodeType == 1) {
                if (elem.firstChild.tagName == elem.nodeName){
                  nodeName = "blockquote";
                }
              }
            break;
          ...
          for (var i = 0; i < attr.length; i++) {
            var item = attr[i];
            var key  = item.nodeName.toLowerCase();
            var data = item.nodeValue;
            if (item.specified) {
              if (key == "style") {
                data = elem.style.cssText;
              }
            }
            if (!item.specified || !data || bkLib.inArray(this.stripAttributes, key) || typeof(data) == "function") {
              continue;
            }
            
            switch (key) {
              case "style":
                var cssText = data.replace(/ /g, "");
                for (itm in this.cssReplace) {
                  if (cssText.indexOf(itm) != -1) {
                    if (elem.tagName != "br" && elem.tagName != "strong") {
                      B.push(this.cssReplace[itm]);
                    }
                    cssText = cssText.replace(itm, "");
                  }
                }
                if (cssText.length>0 && cssText.substr(cssText.length-1) != ";"){
                  cssText += ";";
                }
                style += cssText;
                data = "";
              break;
              case "class":
                data = data.replace("Apple-style-span", "");
              break;
              case "size":
                style += "font-size:" + data + "px;";
                data = "";
              break;
              case "color":
                style += "color:" + data + ";";
          ...
          if((style != "" || a != "") && nodeName != "span") { 
            ...
            if (!hasChildren && !bkLib.inArray(this.noShort, elem.tagName.toLowerCase())) {
              closed = true;
              xhtml.push("/>");
            } else {
              xhtml.push(">");
    ...
    if (hasChildren) {
      ...
      for (var i=0; i<children.length; i++){
        var code = this.toXHTML(children[i], true);
        xhtml.push(code);
      }
    }
    if ((style!= "" || a != "") && nodeName != "span")
    {
      if (nodeType == 1 && noParent && !closed) {
        xhtml.push("</");
        xhtml.push(nodeName);
        xhtml.push(">");
      }
...
var nicBBCode = bkClass.extend({
  ...
  bbGet: function(editor) {
    var xhtml = this.xhtml.toXHTML(editor.getElm());
    try {
      editor.content = this.toBBCode(xhtml);
    } catch(ex){
      alert(ex.toJSONString());
    }
  ...
  toBBCode: function(B) {
    return bbcode.html2bbCode(B);
  },
  fromBBCode: function(A) {
    A = bbcode.bbcode2Html(A);
    return A;
  }
});
