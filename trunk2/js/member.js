  ...
  d.onOK = function () {
    ...
    Ajax.call(url, function(s) {
      if(s.done) {
          ...
          $('add_address_button').innerHTML = '<a href="javascript:addAddress();">'+ lang.add_address + '</a>';
      }
    });
  };
...
function delivered(orderId, storeId){
  ...
  var d = new Dialog(DIALOG_CONFIRM);
  d.onOK = function () {
    Ajax.call('index.php?app=shopping&act=evaluation&order_id='+currOrderId+'&rank=' + $('rank').value + '&is_add_friend='+ ($('is_add_friend').checked ? 1 : 0), function (response) {
      if (!response.done)
      {
        alert(response.msg);
        return;
