 ...
 * ============================================================================
 * $Id: pay800.php 5629 2008-08-21 09:33:58Z Garbin $
 */
...
class pay800 extends Payment
{
    ...
    function get_code($order_obj)
    {
        ...
        $data_M_OrderID     = $order_obj->get_pay_log();                       //订 单 号：
        $data_M_OAmount     = $order['payable'];                 //订单金额：
        $data_M_OCurrency   = CURRENCY == 'CNY' ? 'RMB' : CURRENCY;            //币 种：
        $data_M_URL         = $this->get_respond_url(basename(__FILE__, '.php')); //返回地址：
        $data_M_Language    = $payment['pay800_language'];            //语言选择：

        $data_T_TradeName   = 'Order SN:' . $order['order_sn'];  //$order['order_sn'];              //商品名称：
