 ...
 * ============================================================================
 * $Id: mod.order.php 5685 2008-09-05 06:50:27Z Garbin $
 */
...
class Order extends Model
{
    ...
    function Order($order_id = NULL, $store_id = NULL, $flag_type = 'order_id')
    {
        ...
        case 'order_id':
            ...
            $this->_id && $where[] = 'oi.order_id =' . $this->_id;
            break;
        case 'order_sn':
            $this->_order_sn = empty($order_id) ? NULL : trim($order_id);
            $this->_order_sn && $where[] = 'oi.order_sn =\'' . $this->_order_sn . '\'';
            break;
        }
        $this->_store_id && $where[] = 'oi.store_id =' . $this->_store_id;

        if(!empty($where))
        {
            $this->_sql_where = implode(' AND ', $where);
        }
    }
    ...
    function drop()
    {
        ...
        $GLOBALS['db']->query("DELETE FROM `ecm_order_info` oi WHERE {$this->_sql_where} AND order_status = 7");
