 ...
 * ============================================================================
 * $Id: mng.goods.php 5681 2008-09-04 09:17:43Z Wj $
 */
...
class GoodsManager extends Manager
{
    ...
    function generate_sku ($goods_id)
    {
        ...
        if ($count == 0)
        {
            $pre_sku = 'ECM' . str_repeat('0', 5 - strlen($goods_id)) . $goods_id;
            $sku =  $pre_sku . '-01';
            $skus[$goods_id][] = $sku;
        }
        else
   ...
   function drop ($in)
   {
    ...
    function update_goods_count()
    {
        if ($this->_store_id > 0)
        {
            $sql = "SELECT COUNT(*) FROM `ecm_goods` WHERE store_id = '{$this->_store_id}'";
    ...
    function get_extra_info($goods_ids)
    {
        ...
        /* 获取店铺所在地 */
        $sql = "SELECT s.store_location, g.goods_id FROM `ecm_goods` AS g LEFT JOIN `ecm_store` AS s ON g.store_id=s.store_id WHERE g.goods_id " . db_create_in($goods_ids);
        $query = $GLOBALS['db']->query($sql);
        while ($res = $GLOBALS['db']->fetch_array($query))
        {
