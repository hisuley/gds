 ...
 * ============================================================================
 * $Id: mng.order.php 5586 2008-08-18 08:20:31Z Garbin $
 */
...
class OrderManager extends Manager
{
    ...
    function set_status($order_id, $status_no, $conditions = '')
    {
        ...
        /* 取得符合条件的ID数组 */
        $affected_ids = $GLOBALS['db']->getAll("SELECT order_id,user_id,points,coupon_sn FROM `ecm_order_info` oi WHERE order_id {$order_ids} {$conditions}{$this->_store_limit}");

        /* 取得商品列表 */
        $goods_list = $GLOBALS['db']->getAll("SELECT goods_id, spec_id, goods_number FROM `ecm_order_goods` og LEFT JOIN `ecm_order_info` oi ON og.order_id = oi.order_id WHERE og.order_id {$order_ids} {$conditions}{$this->_store_limit}");

        $order_ids && $GLOBALS['db']->query("UPDATE `ecm_order_info` oi SET order_status={$status_no} WHERE order_id {$order_ids} {$conditions}{$this->_store_limit}");
    ...
    function get_last_order_by_user($user_id)
    {
        ...
        $sql = 'SELECT oi.* FROM `ecm_order_info` oi WHERE oi.user_id=' . $user_id . $this->_store_limit . ' ORDER BY add_time DESC LIMIT 1';
    ...
    function auto_delivered($evaluation_value, $time_range = 1209600)
    {
        ...
        foreach ($orders as $_k => $_o)
        {
            $order_logger->write(array('action_user' => '0',
                                       ...
                                       'action_note' => 'System auto delivered',
                                       'action_time' => $now));
    ...
    function auto_evaluation($evaluation_value, $time_range = 604800)
    {
        ...
        /*
        $sql = 'UPDATE `ecm_order_info` oi LEFT JOIN `ecm_order_action` oa ON oi.order_id=oa.order_id SET buyer_evaluation=' . ORDER_EVALUATION_GOOD . ' WHERE oa.order_status = ' . ORDER_STATUS_DELIVERED . " AND action_time < {$now} - {$time_range}{$this->_store_limit} ";
        */
        include_once(ROOT_PATH . '/includes/models/mod.order.php');
        include_once(ROOT_PATH . '/includes/manager/mng.credit.php');
        foreach ($orders as $_k => $_o)
    ...
    function get_order_num($user_id, $where)
    {
        ...
        $sql = 'SELECT COUNT(*) as order_num '.
               'FROM `ecm_order_info` oi '.
               "WHERE 1{$where}{$this->_store_limit}";
