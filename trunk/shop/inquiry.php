...
//引入语言包
    ...
    $info = $dbo->getRow($sql);
    if (!$info) {
        trigger_error($i_langpackage->i_goods_error);
    } //没有此商品
...
<div class="">
<form action="inquiry.php?app=post&gid=<?php echo $goods_id;?>" method="post" onsubmit="return checkform();">
<TABLE cellSpacing=2 cellPadding=0 width="100%" align=center border=0>
  ...
  <TR>
    ...
    <TD><textarea id="content" name="content" cols="40" rows="10" onkeyup="this.value=this.value.slice(0,300);"></textarea></TD></TR>
  <TR>
...
<script type="text/javascript">
    function checkform(){
    	...
    	var textareac = document.getElementById("content");
    	
    	if(textareac.value.length>300){
    		alert("<?php echo $i_langpackage->i_work_count_error;?>");
    		textareac.focus();
    		return false;
    	}
    	return true;
...
=======
    ...
    <div id="wrapper">
        ...
        <?php include("shop/index_header.php"); ?>
        <!-- 头部 -->
        <div class="path"><?php echo $i_langpackage->i_location; ?>：<a
                href="index.php"><?php echo $i_langpackage->i_index; ?></a>
            > <?php echo $i_langpackage->i_iwantget_price; ?> </div>
        <div class="">
		...
		<input name="goods_name" type="hidden" value="<?php echo $info['goods_name']; ?>"/>
                    ...
                    <TR>
                        <TD align=right><?php echo $i_langpackage->i_askprice_title; ?>：</TD>
                        <TD><input class=InputBorder maxlength=120 size=40 name="title" type="text"
                                   value="<?php echo str_replace("{name}", $info['goods_name'], $i_langpackage->i_inq_msat); ?>"/>
                        </TD>
                    </TR>
                    <TR>
                        <TD align=right valign="top"><?php echo $i_langpackage->i_inq_askinfo; ?>：</TD>
                        <TD><textarea id="content" name="content" cols="40" rows="10"
                                      onkeyup="this.value=this.value.slice(0,300);"></textarea></TD>
                    </TR>
                    <TR>
                        <TD align=right><?php echo $i_langpackage->i_inq_truename; ?>：</TD>
                        <TD><input name="user_truename" type="text" value="<?php echo $user_info['user_truename']; ?>"/>
                        </TD>
                    </TR>
                    <tr>
                        <td align="right"><label><?php echo $i_langpackage->i_inq_email; ?></label>：</td>
                    ...
                    <tr>
                        <td align="right"><?php echo $i_langpackage->i_inq_tel; ?>：</td>
                        <td><input name="user_telphone" type="text" value="<?php echo $user_info['user_telphone']; ?>"/>
                        </td>
                    </tr>
                    <tr>
                        <td align="right"><?php echo $i_langpackage->i_inq_mob; ?>：</td>
                        <td><input name="user_mobile" type="text" value="<?php echo $user_info['user_mobile']; ?>"/>
                        </td>
                    </tr>
                    <TR>
                        <TD style="border-bottom:0px;" align=right> </TD>
                        <TD height="40" style="border-bottom:0px;"><input name="button" type="submit" class="button"
                                                                          value="<?php echo $i_langpackage->i_inq_sendmsg; ?>"/>
                        </TD>
                    </TR>
                </TABLE>
            </form>
