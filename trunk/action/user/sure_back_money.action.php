...
if ($order_info['pay_status'] != '3') {
    action_return(0, $m_langpackage->m_no_sure_back_money);
}

//数据库操作
dbtarget('w', $dbServs);
$dbo = new dbex();
$sql = "update `$t_order_info` set pay_status=4 where order_id='$order_id' and user_id='$user_id'";
if ($dbo->exeUpdate($sql)) {
    action_return(1, $m_langpackage->m_back_money_suc);
} else {
    action_return(0, $m_langpackage->m_back_money_fail, '-1');
}
?>