...
if($file) {
	...
	foreach($file as $k=>$v) {
		if($v['flag']==1) {
			...
			$post1['upl_time'] = $ctime->long_time();
			$post1['img_url'] = str_replace($webRoot,"",$v['dir']).$v['name'];
			
			$img_id = insert_img_size_up_shop($dbo,$t_img_size,$post1,$t_shop_info,$row['count_imgsize']+$v['size'],$shop_id);
