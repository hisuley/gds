...
<<<<<<< HEAD
if(!empty($attr_id) && isset($post_attr['attr_values'][$attr_id['attr_id']])){
...
function filterAttr($haveArray,$postArray) {
    ...
    foreach($haveArray as $key=>$value) {
        if(isset($postArray['attr_values'][$key])) {
            if(empty($postArray['price'][$key]))
                $postArray['price'][$key] = 0;
            ...
            if($postArray['attr_values'][$key] != $value['attr_values'] || $postArray['price'][$key] != $value['price']) {
                if(is_array($postArray['price'][$key])){
                    ...
                    if(empty($value['price']))
                        $value['price'] = 0;
                    $postArray['price'][$key] = intval($value['price']);
=======
function filterAttr($haveArray, $postArray)
{
    $array = array();
