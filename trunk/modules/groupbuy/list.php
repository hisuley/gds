...
<body onload="menu_style_change('groupbuy_list');changeMenu();">
	...
	<div class="site_map">
	  <?php echo $m_langpackage->m_current_position;?><A href="index.php"><?php echo $SYSINFO['sys_name'];?></A>/<a href="modules.php"><?php echo $m_langpackage->m_u_center;?></a>/&nbsp;&nbsp;<?php echo $m_langpackage->m_groupbuy_list;?>
	</div>
    <div class="clear"></div>
        ...
        <div class="main_right">
            ...
            <div class="cont">
                <div class="cont_title"><span class="tr_op"><a href="modules.php?app=groupbuy_add"><?php echo $m_langpackage->m_group_add;?></a></span><?php echo $m_langpackage->m_group_list;?></div>
                <hr />
				...
				<table width="100%" cellspacing="0">
					...
					<tr>
						...
						<?php if($v['examine']!=0&&$v['recommended']==0 and $v['start_time']>= $now_time) {?>
							<a href="do.php?act=groupbuy_release&id=<?php echo  $v['group_id'];?>" onclick="return confirm('<?php echo  $m_langpackage->m_confirm;?>');"><?php echo  $m_langpackage->m_published;?></a><br />
						<?php }?>
						<?php if($v['examine']!=0&&$v['recommended']==1 or $v['end_time']<= $now_time) {?>
							<a href="modules.php?app=shop_my_order&id=<?php echo  $v['group_id'];?>"><?php echo  $m_langpackage->m_order_group;?>(<?php echo $v['order_num'];?>)</a><br />
						<?php }?>
						<?php if($v['examine']!=0) {?>
							<a href="modules.php?app=groupbuy_login&id=<?php echo  $v['group_id'];?>" target="_blank"><?php echo  $m_langpackage->m_order_status;?></a><br />
						<?php }?>
						<?php if($v['examine']!=0&&$v['recommended']==0 and $v['start_time'] <= $now_time and $v['end_time'] >= $now_time) {?>
							<a href="do.php?act=groupbuy_end&id=<?php echo  $v['group_id'];?>" onclick="return con(<?php echo $v['purchase_num'];?>,<?php echo $v['min_quantity'];?>);"><?php echo  $m_langpackage->m_accomplish;?></a><br />
						<?php }?>
						<?php if($v['examine']==0) {?>
					...
					<?php }?>
					<tr><td colspan="7" class="center"><div class="page"><?php  require("modules/page.php");?></div></td></tr>
					<?php  } else {?>
					<tr><td colspan="7" class="center"><?php echo  $m_langpackage->m_nogoods_list;?></td></tr>
					<?php }?>
				</table>
				</form>
		</div>
