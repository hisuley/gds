...
<div class="site_map">
    <?php echo $m_langpackage->m_current_position; ?><A href="index.php"><?php echo $SYSINFO['sys_name']; ?></A>/<a
        href="modules.php"><?php echo $m_langpackage->m_u_center; ?></a>/&nbsp;&nbsp;<?php echo $m_langpackage->m_edit_transport_template; ?>
</div>
<div class="clear"></div>
<?php require("modules/left_menu.php"); ?>
<div class="main_right">
    <div class="right_top"></div>
    <div class="cont">
        <div class="title_uc"><span class="tr_op"><a href="javascript:window.close();"><h5>返回修改页面</h5>
                </a>&nbsp;&nbsp;</span></div>
        <form action="do.php?act=edit_transport_template" method="post" name="form_transport_template"
              onsubmit="return checkForm(this);">
            <input type="hidden" name="id" value="<?php echo $id; ?>"/>
            <table id="boxtable" width="100%" style="border:0" cellspacing="0">
                <tr>
                    <td width="100px;"><?php echo $m_langpackage->m_transport_name; ?>:</td>
                    <td><input type="text" name="transport_name"
                               value="<?php echo $transport_template_info['transport_name']; ?>"/><span><?php echo $m_langpackage->m_transport_name_message; ?></span>
                    </td>
                </tr>
                <tr>
                    <td><?php echo $m_langpackage->m_transport_description; ?>:</td>
                    <td><input type="text" name="description"
                               value="<?php echo $transport_template_info['description']; ?>"/><span><?php echo $m_langpackage->m_transport_description_message; ?></span>
                    </td>
                </tr>
                <tr>
                    <td colspan="3">
                        <b><?php echo $m_langpackage->m_choose_transport_type; ?></b><span><?php echo $m_langpackage->m_choose_transport_type_message; ?></span>
                    </td>
                </tr>
                <?php if (is_array($arr_list)) { ?>
                    <?php foreach ($arr_list as $k => $value) { ?>
                        <tr>
                            <td colspan="3">&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="transport_type[]"
                                                                           value="<?php echo $value['tranid']; ?>"
                                                                           id="<?php echo $value['tranid']; ?>"
                                                                           onclick="showtype('<?php echo $value['tranid']; ?>')"/><?php echo $value['tran_name']; ?>
                                :
                            </td>
                        </tr>
                        <tr id="<?php echo $value['tranid']; ?>_box" style="display:none;">
                            <td colspan="3">&nbsp;&nbsp;&nbsp;&nbsp;
                                <?php echo $m_langpackage->m_transport_template_frist; ?><input type="text"
                                                                                                id="frist<?php echo $value['tranid']; ?>"
                                                                                                name="frist<?php echo $value['tranid']; ?>"
                                                                                                value="0.00"
                                                                                                maxlength="10"/>
                                <?php echo $m_langpackage->m_transport_template_second; ?><input type="text"
                                                                                                 id="second<?php echo $value['tranid']; ?>"
                                                                                                 name="second<?php echo $value['tranid']; ?>"
                                                                                                 value="0.00"
                                                                                                 maxlength="10"/><br/>
                                <ul id="wordbox_<?php echo $value['tranid']; ?>">
                                    <?php echo $type_list[$v['tranid']]; ?>
                                ...
                                &nbsp;&nbsp;&nbsp;&nbsp;
                                <a href="javascript:;"
                                   onclick="showareslist('<?php echo $value['tranid']; ?>')"><b><?php echo $m_langpackage->m_transport_cont; ?></b></a>
                            </td>
                        </tr>
                    <?php } ?>
                <?php } ?>
                <tr>
                    <td colspan="3"><input class="submit" type="submit" name="sub"
                                           value="<?php echo $m_langpackage->m_shure; ?>"/></td>
                </tr>
            </table>
        </form>
    </div>
...
<div id="bgdiv" style="display:none;"></div>
<div id="category_select" style="display:none;">
    <div class="category_title_1"><span
            onclick="hidebgdiv();"><?php echo $m_langpackage->m_close; ?></span><?php echo $m_langpackage->m_plss_select_cateogry; ?>
    </div>
    <ul id="select_first" class="ulselect">
        <?php if (isset($category_info)) { ?>
            <?php foreach ($category_info as $k => $v) { ?>
...
<div style="position:absolute;style:top:0px;left:0px; padding:5px; background:#f1f1f1; display:none; width:420px;"
     id="mydiv">
    <?php if (is_array($area_list)) { ?>
        <?php foreach ($area_list as $k => $value) { ?>
            <div style="float:left; width:75px;"><input type="checkbox" name="area[]"
                                                        value="<?php echo $value['area_id']; ?>"/><?php echo $value['area_name']; ?>
            </div>
        <?php } ?>
    <?php } ?>
    <input type="hidden" name="hdnum" value="0" id="hdnum"/>
