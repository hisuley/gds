...
<div class="site_map">
    <?php echo $m_langpackage->m_current_position; ?><A href="index.php"><?php echo $SYSINFO['sys_name']; ?></A>/<a
        href="modules.php"><?php echo $m_langpackage->m_u_center; ?></a>/&nbsp;&nbsp;<?php echo $m_langpackage->m_goods_list; ?>
</div>
<div class="clear"></div>
<?php require("modules/left_menu.php"); ?>
<div class="main_right">
    ...
    <div class="cont">
        <div class="cont_title">
            	...
            	<a href="modules.php?app=csv_export"><?php echo $m_langpackage->m_csv_export; ?></a>
            	<a href="modules.php?app=csv_import"><?php echo $m_langpackage->m_csv_import; ?></a>
					<a href="modules.php?app=goods_csv_taobao"><?php echo $m_langpackage->m_import_taobao; ?></a><a
                        href="modules.php?app=goods_add"><?php echo $m_langpackage->m_add_goods; ?></a>
				</span><?php echo $m_langpackage->m_goods_list; ?>
        </div>
        <hr/>
        <div class="search">
            <form action="modules.php" method="get" name="search_form" style="float:left;">
                ...
                <input type="text" name="k" value="<?php if (empty($k)) {
                    echo $m_langpackage->m_goods_keyword;
                } else {
                    echo $k;
                } ?>"
                       onfocus="javascript:if(this.value=='<?php echo $m_langpackage->m_goods_keyword; ?>'){this.value=''}"
            ...
            <table width="100%" class="">
                ...
                if (!empty($result['result'])) {
                    foreach ($result['result'] as $v) {
                        ?>
                        <tr class="trcolor">
                            <td class="center" width="40"><input type="checkbox" name="checkbox[]"
                                                                 value="<?php echo $v['goods_id']; ?>"/></td>
                            <td class="center"><a href="goods.php?id=<?php echo $v['goods_id']; ?>" target="_blank"><img
                                        src="<?php echo $v['goods_thumb']; ?>" width="80" height="80"
                                        onerror="this.src='skin/default/images/nopic.gif'"/></a></td>
                            <td align="left" valign="top">
                                <div class="goodsname"><a href="goods.php?id=<?php echo $v['goods_id']; ?>"
                                                          target="_blank"><?php echo $v['goods_name']; ?></a></div>
                                <?php echo $m_langpackage->m_custom_categories; ?>：<span
                                    class="category"><?php if (isset($shop_category_info[$v['ucat_id']])) { ?><?php echo $shop_category_info[$v['ucat_id']]; ?><?php } else { ?><?php echo $m_langpackage->m_undefinition; ?><?php } ?></span>
                                &nbsp; <?php echo $m_langpackage->m_goods_type; ?>：<span
                                    class="category"><?php if (isset($typeinfo[$v['type_id']])) { ?><?php echo $typeinfo[$v['type_id']]; ?><?php } ?></span>
                                <br/>
                                <?php if ($v['lock_flg'] == 0) { ?>
                                    <?php echo $m_langpackage->m_best; ?>:<?php if ($v['is_best']) { ?><img
                                        src="skin/default/images/yes.gif"
                                        onclick="toggle_show(this,'best','<?php echo $v['goods_id']; ?>')" /><?php } else { ?>
                                        <img src="skin/default/images/no.gif"
                                             onclick="toggle_show(this,'best','<?php echo $v['goods_id']; ?>')" /><?php } ?> &nbsp;
                                    <?php echo $m_langpackage->m_promote; ?>:<?php if ($v['is_promote']) { ?><img
                                        src="skin/default/images/yes.gif"
                                        onclick="toggle_show(this,'promote','<?php echo $v['goods_id']; ?>')" /><?php } else { ?>
                                        <img src="skin/default/images/no.gif"
                                             onclick="toggle_show(this,'promote','<?php echo $v['goods_id']; ?>')" /><?php } ?> &nbsp;
                                    <?php echo $m_langpackage->m_new; ?>:<?php if ($v['is_new']) { ?><img
                                        src="skin/default/images/yes.gif"
                                        onclick="toggle_show(this,'new','<?php echo $v['goods_id']; ?>')" /><?php } else { ?>
                                        <img src="skin/default/images/no.gif"
                                             onclick="toggle_show(this,'new','<?php echo $v['goods_id']; ?>')" /><?php } ?> &nbsp;
                                    <?php echo $m_langpackage->m_hot; ?>:<?php if ($v['is_hot']) { ?><img
                                        src="skin/default/images/yes.gif"
                                        onclick="toggle_show(this,'hot','<?php echo $v['goods_id']; ?>')" /><?php } else { ?>
                                        <img src="skin/default/images/no.gif"
                                             onclick="toggle_show(this,'hot','<?php echo $v['goods_id']; ?>')" /><?php } ?>
                                <?php } ?>
                            </td>
                            <td align="left" title="<?php echo $m_langpackage->m_click_editcontent; ?>"
                                id="goodspriceid_<?php echo $v['goods_id']; ?>"><span
                                    onclick="edit_price(this,<?php echo $v['goods_id']; ?>)"><?php echo $v['goods_price']; ?></span>
                            </td>
                            <td style="text-align:center;">
                                <?php if ($v['lock_flg'] == 0) { ?>
                                    <?php if ($v['is_on_sale']) { ?>
                                        <img src="skin/default/images/yes.gif"
                                             onclick="toggle_show(this,'on_sale','<?php echo $v['goods_id']; ?>')"/>
                                    <?php } else { ?>
                                        <img src="skin/default/images/no.gif"
                                             onclick="toggle_show(this,'on_sale','<?php echo $v['goods_id']; ?>')"/>
                                    <?php } ?>
                            ...
                            <td style="text-align:center;" title="<?php echo $m_langpackage->m_click_editcontent; ?>"
                                id="goodssortid_<?php echo $v['goods_id']; ?>">
                                <span
                                    onclick="edit_sort(this,<?php echo $v['goods_id']; ?>)">&nbsp;<?php echo $v['sort_order']; ?>
                                    &nbsp;</span></td>
                            <td style="text-align:center;" title="<?php echo $m_langpackage->m_click_editcontent; ?>"
                                id="goodsnumberid_<?php echo $v['goods_id']; ?>">
                                <span
                                    onclick="edit_number(this,<?php echo $v['goods_id']; ?>)">&nbsp;<?php echo $v['goods_number']; ?>
                                    &nbsp;</span></td>
                            <td style="text-align:center;">
                                <?php if ($v['lock_flg'] == 0) { ?>
                                    <a href="modules.php?app=goods_edit&id=<?php echo $v['goods_id']; ?>"><?php echo $m_langpackage->m_edit; ?></a>
                                    <br/><a
                                    href="modules.php?app=goods_gallery&id=<?php echo $v['goods_id']; ?>" <?php if ($v['is_set_image'] == 0) {
                                        echo '';
                                    } ?>
                                    title="<?php echo $m_langpackage->m_image_upload; ?>"><?php echo $m_langpackage->m_goods_photo; ?></a><?php } ?>
                                <br/><a href="do.php?act=goods_del&id=<?php echo $v['goods_id']; ?>"
                                        onclick="return confirm('<?php echo $m_langpackage->m_sure_delgoods; ?>');"><?php echo $m_langpackage->m_del; ?></a>
                            </td>
                        </tr>
                    <?php } ?>
                    <tr>
                        <td colspan="8" class="page"><?php require("modules/page.php"); ?></td>
                    </tr>
                    <tr>
                        <td colspan="8">
                            <input class="submit" type="submit" name="down"
                                   value="<?php echo $m_langpackage->m_goods_ushelves; ?>"/>
                            <input class="submit" type="submit" name="up"
                                   value="<?php echo $m_langpackage->m_goods_dshelves; ?>"/>
                        </td>
                    </tr>
                <?php } else { ?>
                    <tr>
                        <td colspan="8" class="center"><?php echo $m_langpackage->m_nogoods; ?></td>
                    </tr>
                <?php } ?>
            </table>
        </form>
