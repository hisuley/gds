...
//引入语言包
    ...
    ?><?php if ($result['countpage'] > 0) {
        ?>
        <a class="upPage"
           href="<?php echo 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF'] . $result['preurl']; ?>"><?php echo $m_langpackage->m_page_pre; ?></a>
        <?php if ($first_page == 1) { ?>
            <a href="<?php echo 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF'] . $result['nopage'] . '1'; ?>"><?php echo '1'; ?></a>
            <span>...</span>
        <?php } ?>
        <?php for ($i = $start_page; $i <= $end_page; $i++) { ?>
            <a <?php if ($i == $result['page']) { ?> class="now" <?php } ?>
                href="<?php echo 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF'] . $result['nopage'] . $i; ?>"><?php echo $i; ?></a>
        <?php } ?>
        <?php if ($last_page == 1) { ?>
            <span>...</span><a
                href="<?php echo 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF'] . $result['nopage'] . $result['countpage']; ?>"><?php echo $result['countpage']; ?></a>
        <?php } ?>
        <a class="nextPage"
           href="<?php echo 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF'] . $result['nexturl']; ?>"><?php echo $m_langpackage->m_page_next; ?></a>
    <?php } ?>
<?php } ?>