	       	...
	       	<div class="cont">
				...
				<form action="do.php?act=shop_payment_update" method="post" name="form_shop_pay" onsubmit="return checkForm();">
					<table width="100%" class="form_table">
						<tr>
							<th width="160px;" align="left"><?php echo  $m_langpackage->m_payment_name;?>：</td>
							<th align="left"><?php echo $payment['pay_name'];?><input type="hidden" name="pay_id" value="<?php echo $payment['pay_id'];?>" />
							<input type="hidden" name="shop_payment_id" value="<?php echo $info['shop_payment_id'];?>" /></td>
						</tr>
						<tr>
							<td><?php echo  $m_langpackage->m_payment_desc;?>：</td>
							<td><textarea name="pay_desc" rows="5" cols="40"><?php echo strip_tags($info['pay_desc']);?></textarea> <span class="red">*</span><?php echo  $m_langpackage->m_payment_showuser_pay;?></td>
						</tr>
						<tr>
							<td><?php echo  $m_langpackage->m_payment_enable;?>：</td>
							<td><input type="radio" name="enabled" value="1" <?php if($info['enabled']){?>checked<?php }?>><?php echo  $m_langpackage->m_yes;?> <input type="radio" name="enabled" value="0"  <?php if($info['enabled']==0){?>checked<?php }?>><?php echo  $m_langpackage->m_no;?></td>
						</tr>
						...
						<?php }?>
						<tr>
							<td colspan="2" class="center"><input type="submit" class="submit" value="<?php echo  $m_langpackage->m_payment_config;?>" /></td>
						</tr>
					</table>
			</form>
	        </div>
    ...
    <div class="cont">
        ...
        <form action="do.php?act=shop_payment_update" method="post" name="form_shop_pay" onsubmit="return checkForm();">
            <table width="100%" class="form_table">
                <tr>
                    <th width="160px;" align="left"><?php echo $m_langpackage->m_payment_name; ?>：</td>
                    <th align="left"><?php echo $payment['pay_name']; ?><input type="hidden" name="pay_id"
                                                                               value="<?php echo $payment['pay_id']; ?>"/>
                        <input type="hidden" name="shop_payment_id"
                               value="<?php echo $info['shop_payment_id']; ?>"/></td>
                </tr>
                <tr>
                    <td><?php echo $m_langpackage->m_payment_desc; ?>：</td>
                    <td><textarea name="pay_desc" rows="5"
                                  cols="40"><?php echo strip_tags($info['pay_desc']); ?></textarea> <span
                            class="red">*</span><?php echo $m_langpackage->m_payment_showuser_pay; ?></td>
                </tr>
                <tr>
                    <td><?php echo $m_langpackage->m_payment_enable; ?>：</td>
                    <td><input type="radio" name="enabled" value="1"
                               <?php if ($info['enabled']){ ?>checked<?php } ?>><?php echo $m_langpackage->m_yes; ?>
                        <input type="radio" name="enabled" value="0"
                               <?php if ($info['enabled'] == 0){ ?>checked<?php } ?>><?php echo $m_langpackage->m_no; ?>
                    </td>
                </tr>
                <?php foreach ($config_arr as $k => $v) { ?>
                    <tr>
                        ...
                        <td>
                            <?php if ($config_select[$k]) { ?>
                                <select name="pay_config[<?php echo $k; ?>]">
                                    <?php foreach ($config_select[$k] as $sk => $sv) { ?>
                                        <option value="<?php echo $sk; ?>"
                                                <?php if ($sk == $info[$k]){ ?>selected<?php } ?>><?php echo $sv; ?></option>
                                    <?php } ?>
                                </select>
                            <?php } else { ?>
                                <input type="text" name="pay_config[<?php echo $k; ?>]" value="<?php echo $info[$k]; ?>"
                                       style="width:200px;"/>
                            <?php } ?>
                        </td>
                    </tr>
                <?php } ?>
                <tr>
                    <td colspan="2" class="center"><input type="submit" class="submit"
                                                          value="<?php echo $m_langpackage->m_payment_config; ?>"/></td>
                </tr>
            </table>
        </form>
    </div>
