...
<div class="site_map">
    <?php echo $m_langpackage->m_current_position; ?><A href="index.php"><?php echo $SYSINFO['sys_name']; ?></A>/<a
        href="modules.php"><?php echo $m_langpackage->m_u_center; ?></a>/&nbsp;&nbsp;<?php echo $m_langpackage->m_recver_order; ?>
</div>
<div class="clear"></div>
<?php require("modules/left_menu.php"); ?>
<div class="main_right">
    ...
    <div class="cont">
        ...
        <table width="100%" border="0" cellspacing="0">
            <tr class="center">
                <th colspan="2"><?php echo $m_langpackage->m_order_goods_info; ?></th>
                <th width=""><?php echo $m_langpackage->m_count; ?></th>
                <th width="200"><?php echo $m_langpackage->m_order_orderinfo; ?></th>
                <th width="60"><?php echo $m_langpackage->m_status; ?></th>
                <th width="75"><?php echo $m_langpackage->m_manage; ?></th>
            ...
            if (!empty($result['result'])) {
                foreach ($result['result'] as $v) {
                    ...
                    foreach ($v['order_goods'] as $k => $val) { ?>
                        <tr class="trcolor">
                            <td class=" center" width="82"><a
                                    href="modules.php?app=user_photo_view&id=<?php echo $val['id']; ?>&path=shop_my_order"
                                    target="_blank"><img src="<?php echo $val['goods_thumb']; ?>" width="80" height="80"
                                                         onerror="this.src='skin/default/images/nopic.gif'"/></a></td>
                            <td width="280"><a
                                    href="modules.php?app=user_photo_view&id=<?php echo $val['id']; ?>&path=shop_my_order"
                                    target="_blank" style="color:#0044DD;"><?php echo $val['goods_name']; ?></a><br/>
                                <?php echo $m_langpackage->m_order_shops; ?>：<a
                                    href="shop.php?shopid=<?php echo $val['shop_id']; ?>&app=index"
                                    target="_blank"><?php echo $val['shop_name']; ?></a>
                                <script src="imshow.php?u=<?php echo $val['user_id']; ?>"></script>
                            </td>
                            <!-- 合并  -->
                            <?php if ($k == 0) { ?>
                                <td class="border_l center"
                                    rowspan="<?php echo $num; ?>"><?php echo $v['order_amount']; ?></td>
                                <td class="border_l textleft" rowspan="<?php echo $num; ?>">
                                    <?php echo $m_langpackage->m_order_payids; ?>：
                                    <a href="modules.php?app=shop_order_view&order_id=<?php echo $v['order_id']; ?>"
                                       title="<?php echo $m_langpackage->m_view_orderinfo; ?>"
                                       style="color:#E38016;"><?php echo $v['payid']; ?></a>
                                    <?php if (!empty($v['group_id'])) { ?> <a
                                        href="goods.php?app=groupbuyinfo&id=<?php echo $v['group_id']; ?>" class="green"
                                        target="_blank">[<?php echo $m_langpackage->m_group_buy; ?>]</a>
                                    <?php } ?><br/>
                                    <?php echo $m_langpackage->m_order_ordertime; ?>
                                    ：<?php echo substr($v['order_time'], 0, 16); ?><br/>
                                    <?php echo $m_langpackage->m_buyer_yes; ?>：<?php echo $v['dname'][0]; ?>
                                </td>
                                <td class="center border_l" rowspan="<?php echo $num; ?>">
                                    <?php  if ($val['lock_flg'] == 1) {
                                ...
                                <td class="center border_l" rowspan="<?php echo $num; ?>">
                                    ...
                                    <?php } else { ?>
                                        <?php if ($v['order_status'] == 1 && $v['pay_status'] == 0) { ?>
                                            <a href="do.php?act=shop_order_del&id=<?php echo $v['order_id']; ?>"
                                               onclick="return confirm('<?php echo $m_langpackage->m_sure_thisorder; ?>');"><?php echo $m_langpackage->m_cancelorder; ?></a>
                                            <br/>
                                        <?php } ?>
                                        <?php if ($v['transport_status'] == 0 && $v['order_status'] == 1 && $v['pay_status'] == 1) { ?>
                                            <?php if ($payment_info[$v['pay_id']]['pay_code'] == 'alipay') { ?>
                                                <a href="plugins/alipay/comput.php?id=<?php echo $v['order_id']; ?>"
                                                   target="_blank"><?php echo $m_langpackage->m_sure_shippingnow; ?></a>
                                                <br/>
                                            <?php } else { ?>
                                                <a href="modules.php?app=shop_sure_recevi&order_id=<?php echo $v['order_id']; ?>"
                                                   onclick="javascript:if(confirm('<?php echo $m_langpackage->m_clicksure_s; ?>')){return true;} else {return false;}"><?php echo $m_langpackage->m_sure_shippingnow; ?></a>
                                                <br/>
                                            <?php } ?>
                                        <?php } ?>
                                        <?php if ($v['transport_status'] == 0 && $v['order_status'] == 1 && $v['pay_status'] == 2) { ?>
                                            <a href="modules.php?app=shop_back_money&order_id=<?php echo $v['order_id']; ?>">退款</a>
                                            <br/>
                                        <?php } ?>
                                        <?php if ($v['protect_status'] == 1 || $v['protect_status'] == 2) { ?>
                                            <a href="modules.php?app=shop_protect_rights&id=<?php echo $v['order_id']; ?>"><font
                                        ...
                                        <!-- 订单评价 开始 -->
                                        <?php if ($v['transport_status'] == 1 && $v['order_status'] == 3 && $v['seller_reply'] == 0) { ?>
                                            <a href="modules.php?app=shop_credit_add&id=<?php echo $v['order_id']; ?>&t=seller"><?php echo $m_langpackage->m_evaluate; ?></a>
                                            <br/>
                                        <?php } else if ($v['transport_status'] == 1 && $v['order_status'] == 3 && $v['seller_reply'] == 1) { ?>
                                            <?php echo $m_langpackage->m_already_valuation; ?>
                                        <?php } ?>
                ...
                <?php } ?>
                <tr>
                    <td colspan="6" class="page"><?php require("modules/page.php"); ?></td>
                </tr>
            <?php } else { ?>
                <tr>
