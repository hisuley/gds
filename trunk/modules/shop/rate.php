...
<div class="site_map">
    <?php echo $m_langpackage->m_current_position; ?><A href="index.php"><?php echo $SYSINFO['sys_name']; ?></A>/<a
        href="modules.php"><?php echo $m_langpackage->m_u_center; ?></a>/&nbsp;&nbsp;<?php if ($t == "seller") { ?> <?php echo $m_langpackage->m_shoprate_frombuyer; ?> <?php } else if ($t == "buyer") { ?><?php echo $m_langpackage->m_shoprate_fromseller; ?><?php } else if ($t == "bymain") { ?><?php echo $m_langpackage->m_shoprate_topep; ?><?php } ?>
</div>
<div class="clear"></div>
                ...
                <?php  if (!empty($result['result'])) {
                    foreach ($result['result'] as $v) {
                        ...
                        <tr>
                            ...
                            <td><?php echo $v['evaluate']; ?><?php if ($v['exptime'] and $v['explanation']) { ?>
                                    <br/>[<?php echo $m_langpackage->m_commentate; ?>]<?php echo $v['explanation']; ?><?php } ?>
                                <br/>[<?php echo $v['evaltime']; ?>]
                            </td>
                            <td class="center"><?php echo $v['user_name']; ?></td>
                            <td><a href="<?php echo goods_url($v['goods_id']); ?>" target="_blank"
                                   style="font-size:12px; font-weight:bold; color:#0044DD"
                                   title="<?php echo $v['goods_name']; ?>"><?php echo sub_str($v['goods_name'], 50); ?></a>
                                <br/><?php echo $m_langpackage->m_price; ?>：￥<span
                                    style="color:#FF6600; font-weight:bold;"><?php echo $v['goods_price']; ?></span><?php echo $m_langpackage->m_yuan; ?>
                            </td>
                            <td><?php if (!$v['exptime']) { ?><a
                                    href="modules.php?app=shop_rate_r&id=<?php echo $v['cid']; ?>&t=<?php echo $t; ?>" ><?php echo $m_langpackage->m_commentate; ?></a><?php } ?>
                            </td>
                        </tr>
                    <?php } ?>
                    <tr>
                        <td colspan="5" class="page"><?php require("modules/page.php"); ?></td>
                    </tr>
                <?php } else { ?>
                    <tr>
                        <td colspan="5" class="center"><?php echo $m_langpackage->m_nolist_record; ?></td>
                    </tr>
                <?php } ?>
            </table>
        </form>
