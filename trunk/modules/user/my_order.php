...
=======
                    <select name="state">
                        ...
                        <option value='4' <?php if ($state == "4") {
                            echo "selected";
                        } ?>>已支付
                        </option>
                        <option value='5' <?php if ($state == "5") {
                            echo "selected";
                        } ?>>未发货
                        </option>
                        <option value='6' <?php if ($state == "6") {
                            echo "selected";
                        } ?>>已发货
                        </option>
                        <option value='7' <?php if ($state == "7") {
                            echo "selected";
                        } ?>>已完成
                        </option>
                        <option value='8' <?php if ($state == "8") {
                            echo "selected";
                        } ?>>已发货待确认
                        </option>
                        <option value='9' <?php if ($state == "9") {
                            echo "selected";
                        } ?>>未评价
                        </option>
                    </select>&nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <table class="commodityCart" width="100%" border="0" cellspacing="0">
            ...
            <?php if (!empty($result['result'])) {
                foreach ($result['result'] as $v) {
                    ...
                    <?php $num = count($v['order_goods']);
                    foreach ($v['order_goods'] as $k => $goods) { ?>
                        <tr>
                        <td><img src="<?php echo $goods['goods_thumb']; ?>" alt="<?php echo $goods['goods_name']; ?>"
                                 width="80" width="80" onerror="this.src='skin/default/images/nopic.gif'"/></td>
                        <td><a href="modules.php?app=user_photo_view&id=<?php echo $goods['id']; ?>&path=user_my_order"
                               target="_blank"><?php echo $goods['goods_name']; ?></a></td>
                        <!-- 合并  -->
                        <?php if ($k == 0) { ?>
                            <td class="border_l"
                                rowspan="<?php echo $num; ?>"><?php echo $m_langpackage->m_order_ordertime; ?>
                                ：<?php echo $v['order_time']; ?><br/><?php echo $m_langpackage->m_order_shops; ?>
                                ：<?php echo $v['shop_name']; ?></td>
                            <td class="border_l"
                                rowspan="<?php echo $num; ?>"><?php echo $m_langpackage->m_money_sign; ?><?php echo $v['order_value']; ?></td>
                            <td class="border_l" rowspan="<?php echo $num; ?>">
                                <?php  if ($goods['lock_flg'] == 1) {
                                    echo "<span class='black'>" . $s_langpackage->s_goods_locked . "</span><br />";
                                } else if ($v['open_flg'] == 1) {
                            ...
                            <td class="border_l" rowspan="<?php echo $num; ?>">
                                <?php if ($goods['lock_flg'] == 1) { ?>
                                    <a href="modules.php?app=user_order_view&order_id=<?php echo $v['order_id']; ?>"
                                ...
                                <?php } else { ?>
                                    <?php if ($v['order_status'] == 1 && $v['pay_status'] == 0) { ?>
                                        <a href="do.php?act=user_order_del&id=<?php echo $v['order_id']; ?>"
                                           onclick="return confirm('<?php echo $m_langpackage->m_sure_cancelthisorder; ?>');"><?php echo $m_langpackage->m_cancelorder; ?></a>
                                        <br/>
                                        <a href="modules.php?app=user_payment_message&id=<?php echo $v['order_id']; ?>"><?php echo $m_langpackage->m_pay; ?></a>
                                        <br/>
                                    <?php } ?>
                                    <?php if ($v['order_status'] == 1 && $v['pay_status'] == 1 && $v['transport_status'] == 0) { ?>
                                        <a href="do.php?act=ask_back_money&id=<?php echo $v['order_id']; ?>">申请退款</a>
                                        <br/>
                                    <?php } ?>
                                    <?php if ($v['order_status'] == 1 && $v['pay_status'] == 3) { ?>
                                        <a href="do.php?act=sure_back_money&id=<?php echo $v['order_id']; ?>">确认退款</a>
                                        <br/>
                                    <?php } ?>

                                    <?php if ($v['transport_status'] == 1 && $v['order_status'] == 3 && $v['protect_status'] == 0) { ?>
                                        <a href="modules.php?app=user_protect_rights&id=<?php echo $v['order_id']; ?>">申请维权</a>
                                        <br/>
                                    <?php } ?>

                                    <?php if ($v['protect_status'] != 0 && $v['protect_status'] != 3) { ?>
                                        <a href="do.php?act=user_cancel_protect&id=<?php echo $v['order_id']; ?>"><font
                                                color="red">结束维权</font></a><br/>
                                    <?php } ?>

                                    <?php if ($v['protect_status'] == 1 || $v['protect_status'] == 2) { ?>
                                        <a href="modules.php?app=user_protect_rights&id=<?php echo $v['order_id']; ?>"><font
                                                color="red">查看维权</font></a><br/>
                                    <?php } ?>

                                    <?php if ($v['transport_status'] == 1 && $v['order_status'] == 1) { ?>
                                        <a href="do.php?act=user_order_checkget&id=<?php echo $v['order_id']; ?>"
                                    ...
                                    <?php if ($v['pay_status'] != 0 && $v['pay_status'] != 4 && $v['order_status'] != 3) { ?>
                                        <?php if ($v['complaint'] == 0) { ?>
                                            <a href="modules.php?app=user_complaint&order_id=<?php echo $v['order_id']; ?>"><?php echo $m_langpackage->m_complaints; ?></a>
                                            <br/>
                                        <?php } else { ?>
                                            <?php echo $m_langpackage->m_already_complain; ?><br/>
                                        <?php } ?>
                                    ...
                                    <!-- 订单评价 开始 -->
                                    <?php if ($v['transport_status'] == 1 && $v['order_status'] == 3 && $v['buyer_reply'] == 0) { ?>
                                        <a href="modules.php?app=shop_credit_add&id=<?php echo $v['order_id']; ?>&t=buyer"><?php echo $m_langpackage->m_evaluate; ?></a>
                                        <br/>
                                    <?php } else if ($v['transport_status'] == 1 && $v['order_status'] == 3 && $v['buyer_reply'] == 1) { ?>
                                        <?php echo $m_langpackage->m_already_valuation; ?><br/>
                                    <?php } ?>
            ...
            <?php } else { ?>
                <tr>
                    <td colspan="6" class="center"><?php echo $m_langpackage->m_nolist_record; ?></td>
                </tr>
            <?php } ?>
            </tbody>
        </table>
