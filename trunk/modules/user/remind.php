...
<div class="site_map">
    <?php echo $m_langpackage->m_current_position; ?><A href="index.php"><?php echo $SYSINFO['sys_name']; ?></A>/<a
        href="modules.php"><?php echo $m_langpackage->m_u_center; ?></a>/&nbsp;&nbsp;<?php echo $m_langpackage->m_remind_setting; ?>
</div>
    ...
    <div class="cont">
        ...
        <form action="do.php?act=user_remind_upd" method="post" name="form_profile">
            <table width="100%" border="0" cellspacing="0">
                ...
                <?php foreach ($remind_rs as $val) { ?>
                    <?php if ($val['remind_type'] == 2) { ?>
                        <tr class="center">
                            <td><?php echo $type[$val['remind_type']]; ?></td>
                            <td class="left"><?php echo $val['remind_name']; ?></td>
                            <td><input type="checkbox" name="<?php echo $val['remind_id']; ?>[site]"
                                       value="1" <?php if (isset($remind_user_arr[$val['remind_id']]['site']) && $remind_user_arr[$val['remind_id']]['site'] == '1') {
                                    echo "checked";
                                } ?> /></td>
                            <td><input type="checkbox" name="<?php echo $val['remind_id']; ?>[mail]"
                                       value="1" <?php if (isset($remind_user_arr[$val['remind_id']]['mail']) && $remind_user_arr[$val['remind_id']]['mail'] == '1') {
                                    echo "checked";
                                } ?> <?php if (!$SYSINFO['sys_smtpusermail'] or !$SYSINFO['sys_smtpuser']) {
                                    echo "disabled";
                                } ?> /></td>
                            <!-- <td><input type="checkbox" name="<?php echo  $val['remind_id'];?>[im]" value="1" <?php if(isset($remind_user_arr[$val['remind_id']]['im']) && $remind_user_arr[$val['remind_id']]['im']=='1'){ echo "checked";}?> <?php if($SYSINFO['im_enable']=='false'){ echo "disabled";}?> /></td>
						<td><input type="checkbox" name="<?php echo  $val['remind_id'];?>[mobile]" value="1" <?php if(isset($remind_user_arr[$val['remind_id']]['mobile']) && $remind_user_arr[$val['remind_id']]['mobile']=='1'){ echo "checked";}?> disabled /></td> -->
                ...
                <?php } ?>
                <tr>
                    <td colspan="4" class="center"><input class="submit" type="submit"
                                                          value="<?php echo $m_langpackage->m_remind_setting; ?>"/></td>
                </tr>
            </table>
        </form>
    </div>
