...
<div class="site_map">
    <?php echo $m_langpackage->m_current_position; ?><A href="index.php"><?php echo $SYSINFO['sys_name']; ?></A>/<a
        href="modules.php"><?php echo $m_langpackage->m_u_center; ?></a>/&nbsp;&nbsp;<?php echo $m_langpackage->m_my_remind; ?>
</div>
<div class="clear"></div>
            ...
            <table width="98%" class="form_table">
                ...
                <?php foreach ($remind_rs as $val) { ?>
                    <tr class="center">
                        ...
                        <td><?php if ($val['isread'] == 0) { ?><span
                                style='color:red;'> <?php } ?><?php echo $type[$val['isread']]; ?></td>
                        <td>
                            <?php if ($val['isread'] == 0) { ?><a
                                href="do.php?act=user_remind_info&searchkey=<?php echo $val['rinfo_id']; ?>&updsubmit=updsubmit"><?php echo $m_langpackage->m_mark_read; ?></a> | <?php } ?>
                            <?php if ($val['isread'] != 2) { ?><a
                                href="do.php?act=user_remind_info&searchkey=<?php echo $val['rinfo_id']; ?>&deletesubmit=deletesubmit"
                                onclick="return confirm('<?php echo $m_langpackage->m_confirm; ?>');"><?php echo $m_langpackage->m_del; ?></a><?php } ?>
                            <?php if ($val['isread'] == 2) { ?><?php echo $type[$val['isread']]; ?><?php } ?>
                        </td>
                    </tr>
                <?php } ?>
                <tr>
                    <td colspan="5"><INPUT class="submit"
                                           onclick="return confirm('<?php echo $m_langpackage->m_confirm; ?>');"
                                           type=submit value="<?php echo $m_langpackage->m_mark_read; ?>"
                                           name=updsubmit> <INPUT class="submit"
                                                                  onclick="return confirm('<?php echo $m_langpackage->m_confirm; ?>');"
                                                                  type=submit
                                                                  value="<?php echo $m_langpackage->m_del; ?>"
