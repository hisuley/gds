...
<link rel="stylesheet" type="text/css" href="skin/<?php echo  $SYSINFO['templates'];?>/css/modules.css"><link rel="stylesheet" type="text/css" href="skin/<?php echo  $SYSINFO['templates'];?>/css/layout.css">
	...
	<h3 class="ttlm_02"><?php echo  $m_langpackage->m_order_info;?></h3>
		...
		<!--	<tr>
			...
			<tr>
                ...
                <h3 class="ttlm_02"><?php echo  $m_langpackage->m_order_message;?></h3>
                <form action="do.php?act=user_order_newmessage" method="post" name="order_message">
                    <table class="form_table_02" width="100%" border="0" cellspacing="0">
                            <tr>
                                <th width="100px"><?php echo  $m_langpackage->m_order_message;?>：</th>
                                <td><textarea name="new_message" style="width:280px;height:60px" id="textareac" onkeyup="this.value=this.value.slice(0,300);"></textarea></td>
                            </tr>
                            <tr><input type="hidden" value="<?php echo  $info['order_id'];?>" name="order_id" /><input type="hidden" value="<?php echo  $info['message'];?>" name="message" />
                                <td colspan="2"><span class="tr_op"><input type="submit" name="submit" class="submit" value="<?php echo  $m_langpackage->m_view_order_new_message;?>" /></span></td>
                            </tr>
                    </table>
                </form>
	</div>
...
<head>
    ...
    <style type="text/css">
        ...
        #reg_step ol li.current {
            background: url(skin/default/images/steps_bg.gif) left top no-repeat;
            padding-top: 4px;
            margin-top: -4px;
            color: #FFF;
        }
...
<body onload="menu_style_change('user_my_order');changeMenu();">
<?php require("shop/index_header.php"); ?>
<div class="site_map"> <?php echo $m_langpackage->m_current_position; ?><A
        href="index.php"><?php echo $SYSINFO['sys_name']; ?></A>/<a
        href="modules.php"><?php echo $m_langpackage->m_u_center; ?></a>/&nbsp;&nbsp;<?php echo $m_langpackage->m_view_orderinfo2; ?>
</div>
<div class="clear"></div>
<?php require("modules/left_menu.php"); ?>
<div class="main_right">
...
<table width="100%" border="0" cellspacing="0">
    ...
    <?php foreach ($info['order_goods'] as $v) { ?>
        <tr>
            <td><a href="goods.php?id=<?php echo $v['goods_id']; ?>" target="_blank"><?php echo $v['goods_name']; ?></a>
            </td>
            <td style="text-align:center"><?php echo $v['goods_price']; ?><?php echo $m_langpackage->m_yuan; ?></td>
            <td style="text-align:center"><?php echo $v['order_num']; ?></td>
        </tr>
    <?php } ?>
    <tr>
        <td colspan="3"><?php echo $m_langpackage->m_order_thisbuyprice; ?>：<span><?php echo $info['order_amount']; ?>
                +<?php echo $m_langpackage->m_transport_price; ?>：<?php echo $info['transport_price']; ?>
                =<?php echo $info['order_value']; ?></span><?php echo $m_langpackage->m_yuan; ?></td>
    </tr>
</table>
<h3 class="ttlm_02"><?php echo $m_langpackage->m_order_getsting; ?></h3>
<table class="form_table_02" width="100%" border="0" cellspacing="0">
    ...
    <tr>
        <th><?php echo $m_langpackage->m_address; ?>：</th>
        <td><?php echo $areas[$info['province']]; ?> <?php echo $areas[$info['city']]; ?> <?php echo $areas[$info['district']]; ?> <?php echo $info['address']; ?>
            , <?php echo $info['zipcode']; ?></td>
    </tr>
    <tr>
        <th>联系方式：</th>
        <td><?php echo $m_langpackage->m_mobile; ?>：<?php echo $info['mobile']; ?>
            , <?php echo $m_langpackage->m_telphone; ?>：<?php echo $info['telphone']; ?></td>
    </tr>
    <!--<tr>
				<th ><?php echo  $m_langpackage->m_mobile;?>：</th>
				<td><?php echo  $info['mobile'];?></td>
...
<table class="form_table_02" width="100%" border="0" cellspacing="0">
    ...
    <tr>
        ...
        <td><span class="tr_op">
				<?php if ($info['order_status'] == 1 && $info['pay_status'] == 0) { ?>
                    <a href="do.php?act=user_order_del&id=<?php echo $info['order_id']; ?>"
                       onclick="return confirm('<?php echo $m_langpackage->m_sure_cancelthisorder; ?>');"><?php echo $m_langpackage->m_cancelorder; ?></a>
                    <a href="modules.php?app=user_payment_message&id=<?php echo $info['order_id']; ?>"><?php echo $m_langpackage->m_pay; ?></a>
                <?php } ?>
                <?php if ($info['order_status'] == 1 && $info['pay_status'] == 1 && $info['transport_status'] == 0) { ?>
                    <a href="do.php?act=ask_back_money&id=<?php echo $info['order_id']; ?>">申请退款</a>
                ...
                <?php if ($info['protect_status'] != 0 && $info['protect_status'] != 3) { ?>
                    <a href="do.php?act=user_cancel_protect&id=<?php echo $info['order_id']; ?>"><font
                            color="red">结束维权</font></a>
                <?php } ?>
                ...
                <?php if ($info['transport_status'] == 1 && $info['order_status'] == 1) { ?>
                    <a href="do.php?act=user_order_checkget&id=<?php echo $info['order_id']; ?>"
                       onclick="return confirm('<?php echo $m_langpackage->m_sure_thisgoodsreceive; ?>');"><?php echo $m_langpackage->m_sure_receive; ?></a>
					申请维权<br/>
                <?php } ?>
                <a href="modules.php?app=user_order_view&order_id=<?php echo $info['order_id']; ?>"
                   title="<?php echo $m_langpackage->m_view_orderinfo; ?>"><?php echo $m_langpackage->m_view_orderinfo2; ?></a>
				<!-- 订单投诉 开始 -->
                <?php if ($info['pay_status'] != 0 && $info['pay_status'] != 4 && $info['order_status'] != 3) { ?>
                    <?php if ($info['complaint'] == 0) { ?>
                        <a href="modules.php?app=user_complaint&order_id=<?php echo $info['order_id']; ?>"><?php echo $m_langpackage->m_complaints; ?></a>
...
<form action="do.php?act=user_order_newmessage" method="post" name="order_message">
    <table class="form_table_02" width="100%" border="0" cellspacing="0">
        <tr>
            <th width="100px"><?php echo $m_langpackage->m_order_message; ?>：</th>
            <td><textarea name="new_message" style="width:280px;height:60px" id="textareac"
                          onkeyup="this.value=this.value.slice(0,300);"></textarea></td>
        </tr>
        <tr><input type="hidden" value="<?php echo $info['order_id']; ?>" name="order_id"/><input type="hidden"
                                                                                                  value="<?php echo $info['message']; ?>"
                                                                                                  name="message"/>
            <td colspan="2"><span class="tr_op"><input type="submit" name="submit" class="submit"
                                                       value="<?php echo $m_langpackage->m_view_order_new_message; ?>"/></span>
            </td>
        </tr>
    </table>
</form>
