...
if ($goodsinfo['goods_number'] == 0) {
    trigger_error($s_langpackage->s_kucun_0);
}
if ($goodsinfo['lock_flg']) {
    trigger_error($s_langpackage->s_goods_locked);
}
if (!$goodsinfo) {
    trigger_error($s_langpackage->s_goods_error);
}
...
<div class="main_right">
    <div class="cont">
        ...
        <div id="stepTip" class="clearfix">
            <ul class="list_step1 clearfix">
                <li class="now"><?php echo $m_langpackage->m_u_first; ?>
                    :<br/><?php echo $m_langpackage->m_ordernow_add; ?></li>
        ...
        <form
            action="modules.php?app=user_order&gid=<?php echo implode(',', $goods_id); ?>&v=<?php echo implode(',', $order_num); ?>"
            method="post" name="for<?php echo $m_langpackage->m_profile; ?>" onsubmit="return checkform();">
                ...
                <?php foreach ($address_rs as $val) { ?>
                    <tr>
                        <td class="center"><input type="radio" name="address_id"
                                                  ...
                                                  onclick="changeurl(<?php echo $val['address_id']; ?>);" <?php if ($val['address_id'] == $address_id) { ?> checked <?php } ?> />
                        </td>
