...
function user_remind_read_base($fields="*",$condition="",$by_col="",$order="",$num="",$cache="",$cache_key=""){
	global $tablePreStr;
	...
	$sql=" select $fields from $t_remind_info ";
	$sql.=" where 1=1";
	if($condition!=''){
		$sql.=" $condition ";
	}
...
//用户消息列表
function user_remind_by_buyer($fields="*",$user_id,$by_col="",$order=""){
	$fields=filt_fields($fields);
	$id_str=filt_num_array($user_id);
	if($id_str!=""){
		$condition=" and user_id in ($id_str) ";
	}
	return user_remind_read_base($fields,$condition,$by_col,$order);
}
?>