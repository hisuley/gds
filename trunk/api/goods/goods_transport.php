...
function goods_transport_read_base($fields = "*", $condition = "", $by_col = "", $order = "", $num = "", $cache = "", $cache_key = "")
{
    global $tablePreStr;
    ...
    $sql = " select $fields from $t_goods_transport ";
    $sql .= " where 1=1";
    if ($condition != '') {
        $sql .= " $condition ";
    }
...
//店铺邮费模版
function goods_transport_by_shopid($fields = "*", $shop_id, $by_col = "", $order = "")
{
    ...
    return goods_transport_read_base($fields, $condition, $by_col, $order);
}
