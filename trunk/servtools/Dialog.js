var IMGFOLDERPATH = '../servtools/dialog/';//图片路径配置
...
//	alert("\n"+cw+"\n"+ch+"\n"+sw+"\n"+sh)
    ...
    if (this.ShowMessageRow)this.Top = (ch - this.Height - 30) / 2 + st - 8 - 50;
    if (this.Left == 0) {
    ...
    arr.push("           	<input id='_ButtonAuto_" + this.ID + "'  type='button' value='" + this.Auto + "' style='display:none' >");
    arr.push("           	<input id='_ButtonOK_" + this.ID + "' class='alt_btn' type='button' value='" + this.Ok + "'>");
    arr.push("           	<input id='_ButtonCancel_" + this.ID + "' class='alt_btn' type='button' onclick=\"Dialog.getInstance('" + this.ID + "').close();\" value='" + this.Cancel + "'>");
    arr.push("            </div></td></tr>");
    ...
    this.AutoButton = pw.$("_ButtonAuto_" + this.ID);
    ...
    if (this.Auto != null) {
        this.AutoButton.style.display = "inline";
    }
    if (this.AutoEvent) {
        this.AutoButton.onclick = this.AutoEvent;
    }
    if (!this.AlertFlag) {
        $E.show(bgdiv);
...
Dialog.prototype.addButton = function (id, txt, func) {
    var html = "<input id='_Button_" + this.ID + "_" + id + "' type='button' class='alt_btn' value='" + txt + "'> ";
    var pw = $E.getTopLevelWindow();
...
Dialog.alert_ok = function (msg, func, w, h) {
    Dialog.alert_init(msg, func, w, h, 'icon_ok.gif', '操作成功');
}
Dialog.alert_init = function (msg, func, w, h, alert_ico, mytitle) {
    var pw = $E.getTopLevelWindow()
    var diag = new Dialog("_DialogAlert" + Dialog.AlertNo++);
    diag.ParentWindow = pw;
    diag.Width = w ? w : 300;
    ...
    var arr = [];
    arr.push("<table height='100%' border='0' align='center' cellpadding='10' cellspacing='0'>");
    arr.push("<tr><td align='right'><img id='Icon' src='" + IMGFOLDERPATH + alert_ico + "' width='34' height='34' align='absmiddle'></td>");
    arr.push("<td align='left' id='Message' style='font-size:9pt'>" + msg + "</td></tr></table>");
    var div = doc.createElement("div");
    div.innerHTML = arr.join('');
    doc.body.appendChild(div);
