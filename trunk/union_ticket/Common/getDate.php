...
{
    $coutValue = "";
    if (strlen($linkChar) > 0) {
        $coutValue = date("Y" . $linkChar . "m" . $linkChar . "d");
    } else {
        $coutValue = date("Y年m月d日");
    }
    return $coutValue;
}
...
function getDateYMD_addDay($linkChar, $addDay)
{
    ...
    if (strlen($linkChar) > 0) {
        ...
        $coutValue = date("Y" . $linkChar . "m" . $linkChar . "d", strtotime("$d   +$addDay   day"));
    } else {
        $coutValue = date("Y年m月d日", strtotime("$d   +$addDay   day"));
    }
    return $coutValue;
}
...
function getDateYMDSHM($linkChar)
{
    $coutValue = "";
    if (strlen($linkChar) > 0) {
        $coutValue = date("Y" . $linkChar . "m" . $linkChar . "d H:i:s");
    } else {
        $coutValue = date("Y年m月d日  H:i:s");
    }
    return $coutValue;
}
