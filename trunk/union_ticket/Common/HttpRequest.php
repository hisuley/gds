...
class HttpRequest
{
    ...
    public function open($ip = "", $port = -1)
    {
        ...
        if ($this->useSocket == 1) {
            if (!$this->fp = $socket = socket_create(AF_INET, SOCK_STREAM, 0)) {
                $this->excption = "can not create socket";
                return false;
            } else {
    ...
    {
        if (!$this->fp) {
            $this->excption = "is not a resource id";
            return false;
        }
        ...
        if ($this->useSocket == 1) {
            ...
            do {
                if ($this->timeout > 0) {
                    if (time() - $timestart > $this->timeout) {
                        break;
                    }
                }
                $this->_buffer .= $buffer;
    ...
    private function _geturlinfo()
    {
        ...
        if ($count > 0) {
            ...
            if (isset($result[2]) && $result[2] != "") {
                $this->port = intval($result[3]);
            } else {
                $this->port = 80;
            }
            return true;
        } else {
            $this->excption = "url format error";
            return false;
        }
    }
