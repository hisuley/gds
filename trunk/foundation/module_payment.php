...
function get_one_shopandpayment(&$dbo, $t_shop_payment, $t_payment, $shop_id, $pay_id)
{
    $sql = "select * from `$t_shop_payment`as a,`$t_payment` as b where a.shop_id='$shop_id' and a.pay_id='$pay_id' and a.pay_id=b.pay_id";
    $rowset = $dbo->getRow($sql);
    $info = array();
    $temp = unserialize($rowset['pay_config']);
    if (is_array($temp)) {
