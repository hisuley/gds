				...
				<?php foreach($privilege_list as $key=>$value) { ?>
					<?php if($SYSINFO['sys_domain']){?>
						<input type="checkbox" name="privilege[<?php echo $key; ?>]" value="1" <?php if(isset($privilege[$key]) && $privilege[$key]){echo "checked";} ?> ><?php echo $value['privilege_name']; ?>
							...
							<input class="small-text" type="text" name="privilege_name[<?php echo $key; ?>]" value="<?php if(isset($privilege[$key])) {echo $privilege[$key];} ?>"  onkeyup="this.value=this.value.replace(/[^\d]/g,'')" onafterpaste="this.value=this.value.replace(/[^\d]/g,'')" maxlength="10" />
							<?php } ?>
						<br />
					<?php }else {?>
