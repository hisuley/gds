...
<div id="maincontent">
    <div class="wrap">
        <div class="crumbs"><?php echo $a_langpackage->a_location; ?>
            ...
            &gt;&gt; <?php echo $a_langpackage->a_member_rank_update ?></div>
        <hr/>
        <div class="infobox">
            <h3><span class="left"><?php echo $a_langpackage->a_member_rank_update ?></span> <span class="right"
                ...
                <form action="a.php?act=member_rank_edit" method="post">
                    <table class="form-table">
                        ...
                        <tr>
                            ...
                            <td><input class="small-text" type="text" name="rank_name"
                                       value="<?php echo $info['rank_name']; ?>"></td>
                        </tr>
                        <tr>
                                ...
                                <?php foreach ($privilege_list as $key => $value) { ?>
                                    <?php if ($SYSINFO['sys_domain']) { ?>
                                        <input type="checkbox" name="privilege[<?php echo $key; ?>]"
                                               value="1" <?php if (isset($privilege[$key]) && $privilege[$key]) {
                                            echo "checked";
                                        } ?> ><?php echo $value['privilege_name']; ?>
                                        <?php if ($value['privilege_type']) { ?>
                                            <input class="small-text" type="text"
                                                   name="privilege_name[<?php echo $key; ?>]"
                                                   value="<?php if (isset($privilege[$key])) {
                                                       echo $privilege[$key];
                        ...
                        <tr>
                            ...
                            <td colspan="2"><span class="button-container"><input class="regular-button" type="submit"
                                                                                  name="submit"
                                                                                  value="<?php echo $a_langpackage->a_update_member_rank ?>"/></span>
                            </td>
                        </tr>
                        </tbody>
                    </table>
