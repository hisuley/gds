...
<div id="maincontent">
    ...
    <div class="wrap">
        <div class="crumbs"><?php echo $a_langpackage->a_location; ?>
            ...
            &gt;&gt; <?php echo $a_langpackage->a_user_rank ?></div>
        <hr/>
        <div class="infobox">
            <h3><span class="left"><?php echo $a_langpackage->a_user_rank ?></span><span class="right"
                                                                                         style="margin-right:15px;"><a
                        href="m.php?app=member_rank_add"
                        style="float: right;"><?php echo $a_langpackage->a_add_rank ?></a></span></h3>
            ...
            <div class="content2">
                <form action="a.php?act=member_rank_del" method="post">
                    <table class="list_table">
                        ...
                        ?>
                        <tr style="text-align:center;">
                            <td><input type="checkbox" name="rank_id[]" <?php if ($value['rank_id'] < 5) {
                                    echo "disabled value='' ";
                                } else {
                                    echo ' value="' . $value['rank_id'] . '" ';
                                } ?> /></td>
                            <td><?php echo $value['rank_id']; ?></td>
                            <td align="left">
                                <div
                                    onclick="edit(this,<?php echo $value['rank_id']; ?>,'divlink<?php echo $value['rank_id']; ?>','a.php?act=updateAjax','tablename=user_rank&colname=rank_name&idname=rank_id&idvalue=<?php echo $value['rank_id']; ?>&logcontent=修改用户等级名称&colvalue=',20);"><?php echo $value['rank_name']; ?></div>
            ...
            <td class="center">
                <?php if ($value['rank_id'] > 1) { ?>
                    <a href="m.php?app=member_rank_edit&id=<?php echo $value['rank_id']; ?>"><?php echo $a_langpackage->a_update_privilege ?></a>
                    <a href="a.php?act=member_rank_del&id=<?php echo $value['rank_id']; ?>"
                       onclick="return confirm('<?php echo $a_langpackage->a_rank_del_all_confirm ?>');"><?php echo $a_langpackage->a_delete ?></a>
                <?php } ?>
            </td>
            </tr>
            <?php } ?>
            <tr>
                <td colspan="5"><span class="button-container"><input class="regular-button" type="submit" name=""
