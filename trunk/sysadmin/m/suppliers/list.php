...
<div id="maincontent">
    ...
    <div class="wrap">
        <div class="crumbs"><?php echo $a_langpackage->a_location; ?>
            ...
            &gt;&gt; <?php echo $a_langpackage->a_suppliers_list; ?></div>
        <hr/>
        <div class="seachbox">
            <div class="content2">
                <form action="m.php?app=suppliers_list" name="searchForm" method="get">
                    <table class="form-table">
                    	...
                    	<span style="margin:1px 2px 0px -10px; float:left; color:#000">
                    		<?php echo $a_langpackage->a_suppliers_name; ?>:
                    	</span>

                                <input type="text" class="small-text" name="name" value="<?php echo $name; ?>"
                                       style="width:100px"/>
                                ...
                                <?php echo $a_langpackage->a_suppliers_lock_status; ?>：
                                <select name='shop_open_status'>
                                    <option value="2"><?php echo $a_langpackage->a_shop_lock_status_all; ?></option>
                                    <option
                                        value="1" <?php if ($shop_open_status == '1') echo "selected" ?>><?php echo $a_langpackage->a_lock; ?></option>
                                    <option
                                        value="0" <?php if ($shop_open_status == '0') echo "selected" ?>><?php echo $a_langpackage->a_unlock; ?></option>
                                </select>
                                <?php echo $a_langpackage->a_suppliers_category; ?>：
                                    ...
                                    <?php if ($category_dg) {
                                        foreach ($category_dg as $value) {
                                            if ($value['parent_id'] == '0') {
                                                ?>
                                                <option
                                                    value="<?php echo $value['cat_id']; ?>" <?php if ($shop_categorie == $value['cat_id']) echo "selected" ?>><?php echo $value['cat_name']; ?></option>
                                            <?php } else { ?>
                                                <option
                                                    value="<?php echo $value['cat_id']; ?>" <?php if ($shop_categorie == $value['cat_id']) echo "selected" ?>><?php echo "----|" . $value['cat_name']; ?></option>
                                            <?php
                                ...
                                <input type="hidden" name="app" value="suppliers_list"/>
                            </td>
                            <td>
        ...
        <div class="infobox">
            <h3><span class="left"><?php echo $a_langpackage->a_suppliers_list; ?></span><span class="right"
                                                                                               style="margin-right:15px;"> <a
                        href="m.php?app=suppliers_add"
                        style="float:right;"><?php echo $a_langpackage->a_suppliers_add; ?></a></span></h3>
                ...
                <form action="a.php?act=lock_shop" name="form1" id="form1" method="post">
                    <table class="list_table">
                        ...
                        <tr style="text-align: center;">
                            ...
                            <th align="left"><?php echo $a_langpackage->a_shopkeeper; ?>
                                /<?php echo $a_langpackage->a_shop_name; ?></th>
                        ...
                        <?php if ($result['result']) {
                            foreach ($result['result'] as $value) {
                                ?>
                                <tr style="text-align:center;">
                                    <td width="2px"><input type="checkbox" name="shop_id[]"
                                                           value="<?php echo $value['shop_id']; ?>"/></td>
                                    <td width="40px"><?php echo $value['shop_id']; ?>.</td>
                                    <td width="" align="left"><?php echo $value['user_name'] ?><br/>
                                        <a href="../shop.php?shopid=<?php echo $value['shop_id']; ?>&app=index"
                                           target="_blank"><?php echo $value['shop_name']; ?></a>
                                    ...
                                    <td>
                                        <?php if ($value['shop_commend'] == 1) { ?>
                                            <img src="../skin/default/images/yes.gif"
                                                 onclick="toggle_show(this,'no','<?php echo $value['shop_id']; ?>')"
                                                 style="cursor:pointer;" alt="前台只显示七家店铺"/>
                                        <?php } else { ?>
                                            <img src="../skin/default/images/no.gif"
                                                 onclick="toggle_show(this,'yes','<?php echo $value['shop_id']; ?>')"
                                                 style="cursor:pointer;" alt="前台只显示七家店铺"/>
                                        <?php } ?>
                                        &nbsp;&nbsp;

                                    </td>
                                    <td><?php if ($value['rank_id'] == 1) {
                                            echo "<span>" . $a_langpackage->a_shop_pass_fail . "</span>";
                                        } else {
                                            echo $userrank[$value['rank_id']]['rank_name'];
                                        } ?></td>
                                    <td><?php echo substr($value['shop_creat_time'], 0, 10); ?></td>
                                    <td>
                                        <a href="a.php?act=suppliers_goodsdownsale&id=<?php echo $value['shop_id']; ?>"
                                           onclick="return confirm('<?php echo $a_langpackage->a_goods_out_sale_message1 . $value['shop_name'] . $a_langpackage->a_goods_out_sale_message2; ?>');"><?php echo $a_langpackage->a_goods_out_sale; ?></a><br/>
                                        <a href="m.php?app=member_reinfo&id=<?php echo $value['user_id']; ?>"><?php echo $a_langpackage->a_update_rank; ?></a><br/>
                                        <?php if ($value['lock_flg'] == 0) { ?>
                                            <a href="a.php?act=lock_shop&id=<?php echo $value['shop_id']; ?>&v=1"
                                               onclick="return confirm('<?php echo $a_langpackage->a_free_shop_mess . $value["shop_name"] . $a_langpackage->a_free_shop_mess1; ?>')"><?php echo $a_langpackage->a_lock_shop; ?></a>
                                            <br/>
                                        <?php } else { ?>
                                            <a href="a.php?act=lock_shop&id=<?php echo $value['shop_id']; ?>&v=0"><?php echo $a_langpackage->a_free_shop; ?></a>
                                            <br/>
                                        <?php } ?>
                                        <a href="m.php?app=suppliers_edit&id=<?php echo $value['shop_id']; ?>"><?php echo $a_langpackage->a_suppliers_edit; ?></a>
                                    </td>
                                </tr>
                            <?php } ?>
                            <tr>
							...
							<select name="v">
                                ...
                                <option value="0"><?php echo $a_langpackage->a_free_shop; ?></option>
                                    ...
                                    <span class="button-container"><input class="regular-button" type='button'
                                                                          onclick='jump_diff("a.php?act=shop_goodsdownsale");'
                                                                          value='<?php echo $a_langpackage->a_batch_down; ?>'/></span>
                                </td>
                            </tr>
                        <?php } else { ?>
                            <tr>
