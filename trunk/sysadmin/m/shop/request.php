...
$sql = "select * from `$t_shop_request` r join `$t_shop_info` s on r.user_id=s.user_id where 1";
if ($s && $s != '10') {
    //权限管理
...
<div id="maincontent">
    ...
    <div class="wrap">
        <div class="crumbs"><?php echo $a_langpackage->a_location; ?>
            ...
            &gt;&gt; <?php echo $a_langpackage->a_m_check_commember; ?></div>
        <hr/>
        <div class="seachbox">
            <div class="content2">
                <form action="m.php?app=shop_request" name="searchForm" method="get">
                    <table class="form-table">
                        ...
                        <tr>
                            ...
                            <td width="50px">
                                <select name="s">
                                    <option value='10' <?php if ($s == '10') {
                                        echo "selected";
                                    } ?> ><?php echo $a_langpackage->a_show_all; ?></option>
                                    <option value='1' <?php if ($s == '1') {
                                        echo "selected";
                                    } ?>><?php echo $a_langpackage->a_audit_succ; ?></option>
                                    <option value='100' <?php if ($s == '100') {
                                        echo "selected";
                                    } ?>><?php echo $a_langpackage->a_audit_wait; ?></option>
                                    <option value='2' <?php if ($s == '2') {
                                        echo "selected";
                                    } ?>><?php echo $a_langpackage->a_audit_refuse; ?></option>
                                </select>
                                <input type="hidden" name="app" value="shop_request"/>
                            </td>
                            <td><span class="button-container"><input class="regular-button" type="submit"
                                                                      value="<?php echo $a_langpackage->a_serach; ?>"/></span>
                            </td>
                        </tr>
                        </tbody>
                    </table>
        ...
        <div class="infobox">
            <h3><?php
                if ($s == '10') {
                    echo $a_langpackage->a_show_all;
                }
                if ($s == '1') {
                    echo $a_langpackage->a_audit_succ;
                }
                if ($s == '2') {
                    echo $a_langpackage->a_audit_refuse;
                }
                if ($s == '100') {
                    echo $a_langpackage->a_audit_wait;
                }?></h3>
                ...
                <form action="a.php?act=shop_request" name="form1" method="post">
                    <table class="list_table">
                        ...
                        <tr style="text-align:center;">
                            <th width="2px"><input type="checkbox" onclick="checkall(this,'request_id[]');" value=''/>
                            </th>
                            <th width="20px"><?php echo $a_langpackage->a_ID; ?></th>
                            <th width="" align="left"><?php echo $a_langpackage->a_company_name; ?></th>
                            <th width="145px"><?php echo $a_langpackage->a_certificate_kind; ?>
                                /<?php echo $a_langpackage->a_certificate_code; ?></th>
                        ...
                        <?php if ($result['result']) {
                            foreach ($result['result'] as $value) {
                                ?>
                                <tr style="text-align:center;">
                                    <td><input type="checkbox" name="request_id[]"
                                               value="<?php echo $value['request_id']; ?>"/></td>
                                    <td><?php echo $value['request_id']; ?>.</td>
                                    <td align="left"><?php echo $value['shop_name']; ?></td>
                                    <td><?php echo $value['credit_type']; ?><br/><?php echo $value['credit_num']; ?>
                                    </td>
                                    <td><a href="../<?php echo $value['credit_commercial']; ?>"
                                           target="_blank"><?php echo $a_langpackage->a_look; ?></a></td>
                                    <td><?php echo substr($value['add_time'], 0, 16); ?></td>
                                    <td><?php if ($value['status'] == 1) {
                                            echo "<span class='green'>" . $a_langpackage->a_audit_successed . "</span>";
                                        } elseif ($value['status'] == 2) {
                                            echo "<span class='red'>" . $a_langpackage->a_audit_fail . "</span>";
                                        } else {
                                            echo $a_langpackage->a_audit_wait;
                                        } ?>
                                    </td>
				...
				<span style="margin:14px 14px 0px 0px; float:left;">
					<select name="s">
                        ...
                        <option value="2"><?php echo $a_langpackage->a_no_pass_success; ?></option>
                                    ...
                                    <span class="button-container"><input type="submit" name="" class="regular-button"
