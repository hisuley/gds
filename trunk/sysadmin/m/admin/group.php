...
if ($_POST) {
    ...
    if (!$right) {
        ...
        if (empty($name)) {
            echo "<script type='text/javascript'>ShowMessageBox('$a_langpackage->a_adminnamenot_null','m.php?app=admin_group');</script>";
            exit;
        }
        $sql = "select * from $t_admin_group where group_name='$name'";
        $row = $dbo->getRow($sql);
        if ($row) {
            echo "<script type='text/javascript'>ShowMessageBox('请不要重复输入该管理组名称','m.php?app=admin_group');</script>";
            exit;
        }
    ...
    <div class="wrap">
        <div class="crumbs"><?php echo $a_langpackage->a_location; ?>
            &gt;&gt; <?php echo $a_langpackage->a_m_member_oprate; ?> &gt;&gt; <a
                href=""><?php echo $a_langpackage->a_m_admingroup_set; ?></a></div>
        <hr/>
        <div class="infobox">
            <h3><span class="left"><?php echo $a_langpackage->a_m_admingroup_set; ?></span> <span class="right"
                                                                                                  style="margin-right:15px;"><a
                        href="javascript:new_add('add');"><?php echo $a_langpackage->a_add; ?><?php echo $a_langpackage->a_mana_group; ?></a></span>
            </h3>

            <div class="content2">
                <form action="m.php?app=admin_group" method="post" name="form" onsubmit="return check();">
                    ...
                    <table class="list_table">
                        ...
                        <?php if ($result['result']) {
                            foreach ($result['result'] as $value) {
                                ...
                                <tr style="text-align:center;">
                                    ...
                                    <td align="left" id="group_<?php echo $value['id']; ?>"><span
                                            onclick="edit_group_name(this,<?php echo $value['id']; ?>)"><?php echo $value['group_name']; ?></span>
                                    </td>
                                    <td width="100px">
                                        <?php if ($value['id'] > 1) { ?>
                                            <a href="a.php?act=group_del&id=<?php echo $value['id']; ?>"
                                               onclick="return confirm('<?php echo $a_langpackage->a_mana_del_group; ?>')"><?php echo $a_langpackage->a_delete; ?></a>
                                        <?php } ?>
                        ...
                        <tr id="add" style="display:none; text-align:center;">
                            ...
                            <td><span class="button-container"><input class="regular-button" type="submit" name="submit"
                                                                      value="<?php echo $a_langpackage->a_add; ?><?php echo $a_langpackage->a_mana_group; ?>"></span>
                            </td>
                        </tr>
                        <tr>
