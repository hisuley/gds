...
<div class="crumbs"><?php echo $a_langpackage->a_location; ?>
    ...
    &gt;&gt;<?php echo $a_langpackage->a_order; ?><?php echo $a_langpackage->a_look; ?></div>
<hr/>
<div class="infobox">
<h3><span class="left"><?php echo $a_langpackage->a_order; ?><?php echo $a_langpackage->a_look; ?></span><span
        class="right" style="margin-right:15px;"><a href="m.php?app=order_alllist"
                                                    style="float: right;"><?php echo $a_langpackage->a_alllist_back; ?></a></span>
</h3>

<div class="content2"> <span style="text-align:center;" align="middle">
				<?php if ($info['order_status'] == 1 && $info['pay_status'] == 0) { ?>
                    <a class="btn_ava"
                       onclick="window.location='a.php?act=order_view&order_id=<?php echo $info['order_id']; ?>&state=1'"><span>取消订单</span></a>
                <?php } else { ?>
                    <a class="btn_unava"><span>取消订单</span></a>
                <?php } ?>
        <?php if ($info['pay_status'] == 0 && ($info['order_status'] == 1 || $info['order_status'] == 2)) { ?>
            <a class="btn_ava"
               onclick="window.location='a.php?act=order_view&order_id=<?php echo $info['order_id']; ?>&state=2'"><span>付&nbsp;&nbsp;款</span></a>
        <?php } else { ?>
            <a class="btn_unava"><span>付款</span></a>
        <?php } ?>
        <?php if ($info['transport_status'] == 0 && $info['order_status'] == 2) { ?>
            <a class="btn_ava"
               onclick="window.location='a.php?act=order_view&order_id=<?php echo $info['order_id']; ?>&state=3'"><span>商家发货</span></a>
        <?php } else { ?>
        ...
        <?php if ($info['transport_status'] == 1 && $info['pay_status'] == 1 && ($info['order_status'] == 1 || $info['order_status'] == 2)) { ?>
            <a class="btn_ava"
               onclick="window.location='a.php?act=order_view&order_id=<?php echo $info['order_id']; ?>&state=4'"><span>确认收货</span></a>
        <?php } else { ?>
    ...
    <form action="a.php?act=order_update" method="post" onsubmit="return check();">
        <table class="list_table">
            ...
            <tr style="text-align:center;">
                ...
                <?php if ($goods_promotions['promote_price']){ ?>
                <td>
                    <div
                        onclick="edit(this,<?php echo $goods_promotions['id']; ?>,'divlink<?php echo $goods_promotions['id']; ?>','a.php?act=updateAjax','tablename=goods_promotions&colname=promote_price&idname=id&idvalue=<?php echo $goods_promotions['id']; ?>&logcontent=修改促销商品价格：&colvalue=',8);"><?php echo $goods_promotions['promote_price']; ?></div>
...
?>
<tr>
    <td align="left" colspan="4">&nbsp;&nbsp;<?php echo $a_langpackage->a_order_goal; ?>：<input type="text"
                                                                                                name="order_value"
                                                                                                value="<?php echo $info['order_value']; ?>"/><?php echo $a_langpackage->a_yuan; ?>
        &nbsp;&nbsp;<?php echo $a_langpackage->a_fact_pay; ?>
        ：<?php echo $info['order_actual']; ?><?php echo $a_langpackage->a_yuan; ?></td>
</tr>
</tbody>
</table>
    ...
    <tr>
        ...
        <td><select name="pay_name">
                <?php foreach ($payarr as $val) {
                    ?>
                    <option <?php if ($val['pay_name'] === $info['pay_name']) {
                        echo "selected";
                    } ?>><?php echo $val['pay_name']; ?></option><?php
                }?>
            </select></td>
    </tr>
    <tr>
