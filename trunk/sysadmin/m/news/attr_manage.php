...
if (!$result_cat) {
    trigger_error($a_langpackage->a_add_cat_need . '!');
}
...
<div id="maincontent">
    ...
    <div class="wrap">
        <div class="crumbs"><?php echo $a_langpackage->a_location; ?> &gt;&gt; <?php echo $a_langpackage->a_content; ?>
            &gt;&gt; <?php echo $a_langpackage->a_news_attr_list; ?></div>
        <hr/>
        <div class="infobox">
            <h3><?php echo $a_langpackage->a_news_attr_list; ?></h3>

            <div class="content2">
                <div class="oprate" style="line-height:24px;">&nbsp;&nbsp;选择新闻分类，进行该分类下的新闻属性管理。新闻属性作为同一种类新闻的共有属性供用户选择。
                </div>
                <div class="oprate" style="line-height:50px;"><span style="float:right;">

			<input type="button" class="regular-button" value="<?php echo $a_langpackage->a_extended_parentattr; ?>"
                   onclick="attr_info_extend(<?php echo $parent_id; ?>)"/>&nbsp;&nbsp;
			<input type="button" class="regular-button" value="<?php echo $a_langpackage->a_attr_add; ?>"
                   onclick="attr_info_add()"/>&nbsp;&nbsp;</span>&nbsp;&nbsp;<?php echo $a_langpackage->a_plselect_category; ?>
                    ：
                    <select name="id" onchange="change_cat_id(this.value);">
                        ...
                        <?php foreach ($cat_dg as $value) { ?>
                            <option
                                value="<?php echo $value['cat_id']; ?>" <?php if ($value['cat_id'] == $cat_id) echo "selected"; ?>><?php echo $value['str_pad'] . $value['cat_name']; ?></option>
                        <?php } ?>
                    </select>
                </div>
                    ...
                    <tr id="tr_0" style="display:none; background:#F7C331;">
                        ...
                        <td id="attr_input_type_td" width="500px">
                            ...
                            foreach ($inputtype_arr as $k => $v) {
                                $i++; ?>
                                <input type="radio" name="input_type[0]"value="<?php echo $k; ?>" <?PHP if ('0' == $k) {
                                    echo "checked";
                                } ?> /><?php echo $v; ?>
                                <?php if ($i == 2) {
                                    echo "<br />";
                        ...
                        <td width="60px" align="center"><input type="checkbox" class="small-text" name="selectable[0]"
                                                               value="0" style="width:25px;" maxlength="1"/></td>
                        <td width="60px" align="center"><input type="checkbox" class="small-text" name="price[0]"
                                                               value="1" style="width:25px;"/></td>
                        ...
                        foreach ($attr_info as $value) {
                            ...
                            <tr id="tr_<?php echo $value['attr_id']; ?>">
                                ...
                                <td width="100px"><input type="text" class="small-text" style="width:50px;"
                                                         name="attr_name[<?php echo $value['attr_id']; ?>]"
                                                         value="<?php echo $value['attr_name']; ?>" class="inputtext">
                                </td>
                                <td width="500px">
                                    <?php $i = 0;
                                    foreach ($inputtype_arr as $k => $v) {
                                        $i++; ?>
                                        <input type="radio" name="input_type[<?php echo $value['attr_id']; ?>]"
                                ...
                                <td width="300px"><textarea
                                        name="attr_values[<?php echo $value['attr_id']; ?>]"><?php echo $value['attr_values']; ?></textarea>
                                </td>
                                <td width="60px" align="center"><input type="checkbox" class="small-text"
                                                                       name="selectable[<?php echo $value['attr_id']; ?>]"
                                                                       value="1"
                                                                       style="width:25px;" <?php if ($value['selectable']) {
                                        echo "checked";
                                    } ?> /></td>
                                <td width="60px" align="center"><input type="checkbox" class="small-text"
                                                                       name="price[<?php echo $value['attr_id']; ?>]"
                                                                       value="1"
                                                                       style="width:25px;" <?php if ($value['price']) {
                                        echo "checked";
                                    } ?> /></td>
                                <td width="60px" align="center"><input type="text" class="small-text" class="small-text"
                                                                       name="sort_order[<?php echo $value['attr_id']; ?>]"
                                ...
                                <td width="175px" align="center">
                                    <input type="button" class="regular-button"
                                           ...
                                           name="btn[<?php echo $value['attr_id']; ?>]"
                                           onclick="attr_info_save(<?php echo $value['attr_id']; ?>);"/>
                                    <input type="button" class="regular-button"
                                           value="<?php echo $a_langpackage->a_delete; ?>"
                                           name="delbtn[<?php echo $value['attr_id']; ?>]"
                                           onclick="attr_info_del(<?php echo $value['attr_id']; ?>)">
                                </td>
                            </tr>
                        <?php }
                    } else { ?>
