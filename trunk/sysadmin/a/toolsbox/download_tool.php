...
if (preg_match("/error\_\d/", $tool_list)) {
    echo '<script type="text/javascript">alert("' . $er_langpackage->{"er_" . $tool_list} . '");window.history.go(-1);</script>';
    exit;
}
    ...
    if (preg_match("/\.sql/", $file_url)) {
        ...
        foreach ($queries as $query) {
            ...
            if (!$dbo->exeUpdate($query)) {
                $result = '<font color="red">' . $t_langpackage->t_sql_false . '</font>';
                exit;
            }
        }
    }
...
/** 添加log */
$admin_log = $a_langpackage->a_m_download_tools; //"下载工具";
...
if (!$xmlDom->validate()) {
    echo '<script type="text/javascript">alert("' . $t_langpackage->t_isset_tool . '");window.history.go(-1);</script>';
    exit;
}
