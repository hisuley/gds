...
/* post 数据处理 */
$post['type_content'] = trim(short_check(get_args('type_content')));
...
dbtarget('w',$dbServs);
$dbo = new dbex;
$type_count = count_complaint_type_byid($dbo,$t_complaint_type,$post['type_content'],$type_id);
if($type_count[0]>0)
	action_return(0,$a_langpackage->a_exist_complaint_type,'-1');

if($type_id!=0){
	$upd_suc=update_complaint_type_byid($dbo,$t_complaint_type,$post,$type_id);
	if($upd_suc) {
		admin_log($dbo,$t_admin_log,$sn = $a_langpackage->a_modify_title);
	...
	if($insert_id) {
		admin_log($dbo,$t_admin_log,$sn = $a_langpackage->a_add_a_complaint_title);
